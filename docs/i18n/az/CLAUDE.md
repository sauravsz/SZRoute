...
npm run dev                    # Dev server http://localhost:21128
...
**SZRoute** — birləşdirilmiş AI proxy/router. Bir uç nöqtə, 160+ LLM təminatçısı, avtomatik geri dönmə.
...
SZRoute-un üç əlaqəli, lakin fərqli müvəqqəti uğursuzluq mexanizmi var. Marşrut davranışını düzəldərkən onların
...
- **İdxallar**: xarici → daxili (`@/`, `@szroute/open-sse`) → nisbət
...
- **Yol aliasları**: `@/*` → `src/`, `@szroute/open-sse` → `open-sse/`, `@szroute/open-sse/*` → `open-sse/*`
- **Default port**: 21128 (API + dashboard eyni portda)
- **Məlumat qovluğu**: `DATA_DIR` env var, varsayılan `~/.szroute/`
