...
npm run dev                    # Сервер розробки на http://localhost:21128
...
**SZRoute** — єдиний AI проксі/маршрутизатор. Один кінцевий пункт, 160+ постачальників LLM, автоматичне резервування.
...
SZRoute має три пов'язані, але різні механізми тимчасових збоїв. Тримайте їх
...
- **Імпорти**: зовнішні → внутрішні (`@/`, `@szroute/open-sse`) → відносні
...
- **Псевдоніми шляхів**: `@/*` → `src/`, `@szroute/open-sse` → `open-sse/`, `@szroute/open-sse/*` → `open-sse/*`
- **Порт за замовчуванням**: 21128 (API + панель управління на одному порту)
- **Директорія даних**: `DATA_DIR` змінна середовища, за замовчуванням `~/.szroute/`
