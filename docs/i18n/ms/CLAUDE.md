...
npm run dev                    # Pelayan dev di http://localhost:21128
...
**SZRoute** — proksi/router AI yang bersatu. Satu titik akhir, 160+ penyedia LLM, auto-fallback.
...
SZRoute mempunyai tiga mekanisme kegagalan sementara yang berkaitan tetapi berbeza. Pastikan skop mereka terpisah semasa menyahpepijat tingkah laku penghalaan. Lihat
...
- **Import**: luaran → dalaman (`@/`, `@szroute/open-sse`) → relatif
...
- **Alias laluan**: `@/*` → `src/`, `@szroute/open-sse` → `open-sse/`, `@szroute/open-sse/*` → `open-sse/*`
- **Port lalai**: 21128 (API + papan pemuka pada port yang sama)
- **Direktori data**: `DATA_DIR` env var, lalai kepada `~/.szroute/`
