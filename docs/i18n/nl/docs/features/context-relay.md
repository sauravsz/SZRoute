...
- before the active account is exhausted, SZRoute generates a compact structured summary
- after authentication selects a different account for the same session, SZRoute injects
...
If the active provider is enabled in `handoffProviders`, SZRoute generates a structured
...
SZRoute prepends the stored handoff as a system message. Injection happens only after the
...
At injection time, SZRoute converts that payload into a `<context_handoff>` system
