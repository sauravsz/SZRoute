...
npm run dev                    # سرور توسعه در http://localhost:21128
...
**SZRoute** — پروکسی/روتر AI یکپارچه. یک نقطه انتهایی، بیش از 160 ارائه‌دهنده LLM، بازگشت خودکار.
...
SZRoute سه مکانیزم موقت شکست مرتبط اما متمایز دارد. دامنه آن‌ها را هنگام اشکال‌زدایی رفتار مسیریابی جدا نگه دارید. برای یک نمای کلی، به
...
- **واردات**: خارجی → داخلی (`@/`, `@szroute/open-sse`) → نسبی
...
- **آلیاس‌های مسیر**: `@/*` → `src/`، `@szroute/open-sse` → `open-sse/`، `@szroute/open-sse/*` → `open-sse/*`
- **پورت پیش‌فرض**: 21128 (API + داشبورد در همان پورت)
- **دایرکتوری داده**: متغیر محیطی `DATA_DIR`، به طور پیش‌فرض به `~/.szroute/`
