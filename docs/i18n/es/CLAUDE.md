...
npm run dev                    # Servidor de desarrollo en http://localhost:21128
...
**SZRoute** — proxy/router de IA unificado. Un punto final, más de 160 proveedores de LLM, retroceso automático.
...
SZRoute tiene tres mecanismos de falla temporal relacionados pero distintos. Mantenga su
...
- **Importaciones**: externo → interno (`@/`, `@szroute/open-sse`) → relativo
...
- **Alias de ruta**: `@/*` → `src/`, `@szroute/open-sse` → `open-sse/`, `@szroute/open-sse/*` → `open-sse/*`
- **Puerto predeterminado**: 21128 (API + dashboard en el mismo puerto)
- **Directorio de datos**: variable de entorno `DATA_DIR`, por defecto `~/.szroute/`
...
16. Nunca incluyas trailers `Co-Authored-By` que acrediten a un asistente de IA, LLM o cuenta automatizada (p. ej. nombres que contengan "Claude", "GPT", "Copilot", "Bot"; correos en `anthropic.com` / `openai.com` / direcciones `noreply.github.com` propiedad de bots). Tales trailers redirigen la atribución del commit a la cuenta del bot en GitHub, ocultando al autor real (`diegosouzapw`) en el historial del PR. Los colaboradores humanos — incluyendo autores de PRs upstream y reporteros de issues que se portan a SZRoute — PUEDEN y DEBEN ser acreditados con trailers estándar `Co-authored-by: Name <email>`; los flujos de trabajo de port upstream (`/port-upstream-features`, `/port-upstream-issues`) dependen de esto.
