...
npm run dev                    # Kehityspalvelin osoitteessa http://localhost:21128
...
**SZRoute** — yhtenäinen AI-proxy/reititin. Yksi päätepiste, yli 160 LLM-toimittajaa, automaattinen varajärjestelmä.
...
SZRoute:lla on kolme liittyvää mutta erilaista tilapäisen epäonnistumisen mekanismia. Pidä niiden
...
- **Tuonnit**: ulkoiset → sisäiset (`@/`, `@szroute/open-sse`) → suhteelliset
...
- **Polkualias**: `@/*` → `src/`, `@szroute/open-sse` → `open-sse/`, `@szroute/open-sse/*` → `open-sse/*`
- **Oletusportti**: 21128 (API + dashboard samalla portilla)
- **Tietohakemisto**: `DATA_DIR` ympäristömuuttuja, oletuksena `~/.szroute/`
