...
npm run dev                    # 开发服务器在 http://localhost:21128
...
**SZRoute** — 统一的 AI 代理/路由器。一个端点，160+ LLM 提供者，自动回退。
...
SZRoute 有三种相关但不同的临时故障机制。在调试路由行为时，请保持它们的范围分开。请参见
...
- **路径别名**：`@/*` → `src/`，`@szroute/open-sse` → `open-sse/`，`@szroute/open-sse/*` → `open-sse/*`
- **默认端口**：21128（API + 仪表板在同一端口）
- **数据目录**：`DATA_DIR` 环境变量，默认为 `~/.szroute/`
