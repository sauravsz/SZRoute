...
npm run dev                    # שרת פיתוח ב-http://localhost:21128
...
**SZRoute** — פרוקסי/נתב AI מאוחד. נקודת קצה אחת, 160+ ספקי LLM, חזרה אוטומטית.
...
SZRoute יש שלושה מנגנוני כישלון זמניים הקשורים אך שונים. שמור על התחום שלהם נפרד כאשר אתה מדבג התנהגות נתיב. ראה את
...
- **ייבוא**: חיצוני → פנימי (`@/`, `@szroute/open-sse`) → יחסי
...
- **Alias נתיב**: `@/*` → `src/`, `@szroute/open-sse` → `open-sse/`, `@szroute/open-sse/*` → `open-sse/*`
- **פורט ברירת מחדל**: 21128 (API + לוח מחוונים באותו פורט)
- **ספריית נתונים**: משתנה סביבה `DATA_DIR`, ברירת מחדל ל-`~/.szroute/`
