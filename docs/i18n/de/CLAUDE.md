...
npm run dev                    # Entwicklungsserver unter http://localhost:21128
...
**SZRoute** — einheitlicher KI-Proxy/Router. Ein Endpunkt, 160+ LLM-Anbieter, automatischer Fallback.
...
SZRoute hat drei verwandte, aber unterschiedliche Mechanismen für temporäre Fehler. Halten Sie ihren
...
- **Imports**: extern → intern (`@/`, `@szroute/open-sse`) → relativ
...
- **Pfad-Aliase**: `@/*` → `src/`, `@szroute/open-sse` → `open-sse/`, `@szroute/open-sse/*` → `open-sse/*`
- **Standardport**: 21128 (API + Dashboard am selben Port)
- **Datenverzeichnis**: `DATA_DIR` Umgebungsvariable, standardmäßig `~/.szroute/`
