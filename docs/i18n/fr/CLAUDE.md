...
npm run dev                    # Serveur de développement à http://localhost:21128
...
**SZRoute** — proxy/router AI unifié. Un point de terminaison, 160+ fournisseurs LLM, retour automatique.
...
SZRoute a trois mécanismes de défaillance temporaire liés mais distincts. Gardez leur portée séparée lors du débogage du comportement de routage. Voir le
...
- **Imports** : externe → interne (`@/`, `@szroute/open-sse`) → relatif
...
- **Alias de chemin** : `@/*` → `src/`, `@szroute/open-sse` → `open-sse/`, `@szroute/open-sse/*` → `open-sse/*`
- **Port par défaut** : 21128 (API + tableau de bord sur le même port)
- **Répertoire de données** : variable d'environnement `DATA_DIR`, par défaut `~/.szroute/`
