...
npm run dev                    # Dev server på http://localhost:21128
...
**SZRoute** — samlet AI proxy/router. Én endpoint, 160+ LLM udbydere, auto-fallback.
...
SZRoute har tre relaterede, men distinkte mekanismer til midlertidige fejl. Hold deres
...
- **Imports**: ekstern → intern (`@/`, `@szroute/open-sse`) → relativ
...
- **Sti aliaser**: `@/*` → `src/`, `@szroute/open-sse` → `open-sse/`, `@szroute/open-sse/*` → `open-sse/*`
- **Standardport**: 21128 (API + dashboard på samme port)
- **Data katalog**: `DATA_DIR` miljøvariabel, standard til `~/.szroute/`
