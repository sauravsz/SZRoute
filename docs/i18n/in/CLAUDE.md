...
npm run dev                    # Server dev di http://localhost:21128
...
**SZRoute** — proxy/router AI terpadu. Satu endpoint, 160+ penyedia LLM, auto-fallback.
...
SZRoute memiliki tiga mekanisme kegagalan sementara yang terkait tetapi berbeda. Jaga agar ruang lingkup mereka terpisah saat melakukan debugging perilaku pengalihan. Lihat diagram
...
- **Impor**: eksternal → internal (`@/`, `@szroute/open-sse`) → relatif
...
- **Alias jalur**: `@/*` → `src/`, `@szroute/open-sse` → `open-sse/`, `@szroute/open-sse/*` → `open-sse/*`
- **Port default**: 21128 (API + dashboard di port yang sama)
- **Direktori data**: variabel lingkungan `DATA_DIR`, default ke `~/.szroute/`
