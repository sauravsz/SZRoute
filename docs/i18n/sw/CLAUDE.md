...
npm run dev                    # Seva ya maendeleo katika http://localhost:21128
...
**SZRoute** — proxy/router ya AI iliyounganishwa. Kipengele kimoja, watoa huduma 160+, auto-fallback.
...
SZRoute ina mitambo mitatu inayohusiana lakini tofauti ya kushindwa kwa muda. Hifadhi upeo wao tofauti unapofanya ufuatiliaji wa tabia ya mwelekeo. Tazama
...
- **Maaliko**: nje → ndani (`@/`, `@szroute/open-sse`) → ya uhusiano
...
- **Majina ya njia**: `@/*` → `src/`, `@szroute/open-sse` → `open-sse/`, `@szroute/open-sse/*` → `open-sse/*`
- **Bandari ya kawaida**: 21128 (API + dashibodi kwenye bandari moja)
- **Direktori ya data**: `DATA_DIR` env var, inarudiwa kwa `~/.szroute/`
