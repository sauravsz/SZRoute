...
title: "Arquitetura do SZRoute"
version: 3.8.2
lastUpdated: 2026-05-13

---

# Arquitetura do SZRoute
...
SZRoute é um gateway de roteamento de IA local e um painel construído sobre Next.js.  
...
flowchart LR
    ...
    subgraph Router[Processo Local SZRoute]
...
- Construtor de metadados de resposta SZRoute: `src/domain/szrouteResponseMeta.ts`
...
- arquivo: `${DATA_DIR}/storage.sqlite` (ou `$XDG_CONFIG_HOME/szroute/storage.sqlite` quando definido, caso contrário `~/.szroute/storage.sqlite`)
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Runtime do SZRoute]
...
> SZRoute v3.8.0. Para a lista canônica e continuamente atualizada, consulte
...
- resposta final do cliente retornada pelo SZRoute; respostas transmitidas são armazenadas na mesma forma de resumo compacto
...
1. `usageDb` e `localDb` compartilham a mesma política de diretório base (`DATA_DIR` -> `XDG_CONFIG_HOME/szroute` -> `~/.szroute`) com migração de arquivos legados.
...
5. O diretório `open-sse/` é publicado como o pacote de **workspace npm** `@szroute/open-sse`. O código-fonte o importa via `@szroute/open-sse/...` (resolvido pelo Next.js `transpilePackages`). Os caminhos de arquivo neste documento ainda usam o nome do diretório `open-sse/` para consistência.
...
- Construir imagem Docker: `docker build -t szroute .`
- Iniciar serviço e verificar:
- `GET /api/settings`
- `GET /api/v1/models`
- A URL base do alvo da CLI deve ser `http://<host>:21128/v1` quando `PORT=21128`
