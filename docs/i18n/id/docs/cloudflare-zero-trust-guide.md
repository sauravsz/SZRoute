...
Panduan ini mendokumentasikan standar infrastruktur jaringan terbaik untuk mengamankan **SZRoute** dan mengekspos aplikasi Anda ke internet secara aman, **tanpa membuka satu pun port (Zero Inbound)**.

## Apa yang Telah Dilakukan pada VM Anda?

Kami mengaktifkan SZRoute dalam mode **Split-Port** melalui PM2:

- **Port \`21128\`:** Menjalankan **hanya API** `/v1`.
...
3. Klik **Add a Tunnel**, pilih **Cloudflared**, dan beri nama `SZRoute-VM`.
...
- **Service Type:** `HTTP`
- **URL:** `127.0.0.1:21128` _(Port internal API)_

### Rute 2: Panel Zero Trust (Tertutup)

- **Subdomain:** `szroute` atau `panel`
...
2. Pilih **Self-hosted**.
3. Di **Application name**, masukkan `Panel SZRoute`.
4. Di **Application domain**, masukkan `szroute.domainanda.com` (sama dengan yang Anda buat di "Rute 2").
5. Klik **Next**.
...
3. Klik **Create rule**.
4. **Name:** `Anti-Penyalahgunaan SZRoute API`
...
2. SZRoute hanya berkomunikasi melalui HTTPS keluar (`cloudflared`) dan tidak menerima koneksi TCP langsung dari internet.
