# Panduan Deployment SZRoute di Fly.io
...
Dokumen ini menjelaskan metode deployment SZRoute di Fly.io yang telah terbukti berhasil, mencakup dua skenario utama:
...
Dokumen ini disusun berdasarkan konfigurasi yang telah diverifikasi pada proyek saat ini, dengan nama aplikasi `szroute`.
...
- Alamat akses: `https://szroute.fly.dev/`
...
- `app = 'szroute'` menentukan aplikasi Fly mana yang menjadi target deployment
...
- Jika sebelumnya Anda menggunakan nama lain seperti `oroute`, jangan sampai tertukar dengan `szroute`
...
Jika Anda sudah mengganti nama aplikasi, ganti `szroute` dengan nama Anda.
...
Parameter-parameter berikut telah digunakan secara nyata pada aplikasi `szroute` saat ini:
...
- Berlaku untuk proyek saat ini yaitu `szroute`
...
```powershell
flyctl secrets set INITIAL_PASSWORD=kata-sandi-kuat-anda -a szroute
```
...
- Apakah aplikasi yang sedang dilihat adalah `szroute`
...
Jika repositori saat ini adalah fork dan Anda ingin menyinkronkan pembaruan dari upstream `https://github.com/sauravsz/SZRoute`, ikuti alur berikut.
...
- Anda membuka aplikasi yang salah, misalnya `oroute`, bukan `szroute`
