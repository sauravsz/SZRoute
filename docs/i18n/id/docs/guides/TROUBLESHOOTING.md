...
Masalah umum dan solusinya untuk SZRoute.
...
| Dashboard terbuka di port yang salah                | Atur `PORT=21128` dan `NEXT_PUBLIC_BASE_URL=http://localhost:21128`                                                                                              |
| Tidak ada log yang ditulis ke disk                  | Atur `APP_LOG_TO_FILE=true` dan pastikan pengambilan log panggilan diaktifkan                                                                                    |
| EACCES: permission denied                           | Atur `DATA_DIR=/path/to/writable/dir` untuk mengganti `~/.szroute`                                                                                            |
...
| `dlopen` / `slice is not valid mach-o file` (macOS) | Jalankan `cd $(npm root -g)/szroute/app && npm rebuild better-sqlite3 && szroute` — lihat [Pembangunan ulang modul native macOS](#macos-native-module-rebuild) di bawah |
...
**Penyebab:** Anda menjalankan versi Node.js di luar batas runtime aman yang disetujui SZRoute. Kasus paling umum adalah menjalankan Node 20, 22, atau 24 versi patch lama yang berada di bawah batas keamanan yang diperlukan SZRoute.
...
3. Instal ulang SZRoute: `npm install -g szroute`
4. Mulai ulang: `szroute`
...
**Solusi (v3.5.5+):** SZRoute sekarang menggunakan fungsi `fetch()` milik undici sendiri ketika dispatcher proxy aktif, memastikan perilaku yang konsisten. Perbarui ke v3.5.5+.
...
SZRoute memperbarui token secara otomatis. Jika masalah berlanjut:
...
1. Pastikan `BASE_URL` mengarah ke instans yang sedang berjalan (misalnya, `http://localhost:21128`)
2. Pastikan `CLOUD_URL` mengarah ke endpoint cloud Anda (misalnya, `https://szroute.dev`)
...
1. Periksa kolom runtime: `curl http://localhost:21128/api/cli-tools/runtime/codex | jq`
...
Ketika banyak permintaan bersamaan mengenai penyedia yang dibatasi lajunya, SZRoute menggunakan mutex + auto rate-limiting untuk membuat serialisasi permintaan dan mencegah kegagalan berantai. Ini berjalan otomatis untuk penyedia dengan API key.

---

## Taksonomi Kegagalan RAG / LLM Opsional (16 masalah)

Beberapa pengguna SZRoute menempatkan gateway di depan tumpukan RAG atau agen. Dalam pengaturan tersebut, umum terjadi pola yang aneh: SZRoute terlihat sehat (penyedia aktif, profil routing baik, tidak ada peringatan batas laju) tetapi jawaban akhir masih salah.
...
1. Saat Anda menyelidiki respons yang buruk, kumpulkan:
   ...
   - route atau combo penyedia di SZRoute
...
3. Simpan nomornya di dashboard, runbook, atau pelacak insiden Anda sendiri di samping log SZRoute.
...
Anda dapat mengabaikan bagian ini jika Anda tidak menjalankan pipeline RAG atau agen di belakang SZRoute.
