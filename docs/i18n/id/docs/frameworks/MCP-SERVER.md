# Dokumentasi Server MCP SZRoute (Bahasa Indonesia)
...
SZRoute MCP sudah tersedia secara bawaan. Jalankan dengan:
...
| Alat                            | Deskripsi                                                         |
| :------------------------------ | :---------------------------------------------------------------- |
| `szroute_get_health`          | Kesehatan gateway, pemutus sirkuit, uptime                        |
| `szroute_list_combos`         | Semua combo yang dikonfigurasi beserta modelnya                   |
| `szroute_get_combo_metrics`   | Metrik performa untuk combo tertentu                              |
| `szroute_switch_combo`        | Ganti combo aktif berdasarkan ID/nama                             |
| `szroute_check_quota`         | Status kuota per penyedia atau semua penyedia                     |
| `szroute_route_request`       | Kirim penyelesaian chat melalui SZRoute                         |
| `szroute_cost_report`         | Analitik biaya untuk periode waktu tertentu                       |
| `szroute_list_models_catalog` | Katalog model lengkap beserta kemampuannya                        |

## Alat Lanjutan (8)

| Alat                               | Deskripsi                                                              |
| :--------------------------------- | :--------------------------------------------------------------------- |
| `szroute_simulate_route`         | Simulasi routing percobaan dengan pohon fallback                       |
| `szroute_set_budget_guard`       | Anggaran sesi dengan tindakan degrade/block/alert                      |
| `szroute_set_resilience_profile` | Terapkan preset conservative/balanced/aggressive                       |
| `szroute_test_combo`             | Uji langsung semua model dalam combo melalui permintaan upstream nyata |
| `szroute_get_provider_metrics`   | Metrik terperinci untuk satu penyedia                                  |
| `szroute_best_combo_for_task`    | Rekomendasi kesesuaian tugas beserta alternatifnya                     |
| `szroute_explain_route`          | Jelaskan keputusan routing yang lalu                                   |
| `szroute_get_session_snapshot`   | Status sesi lengkap: biaya, token, kesalahan                           |
