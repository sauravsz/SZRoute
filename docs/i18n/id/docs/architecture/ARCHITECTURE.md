# Arsitektur SZRoute (Bahasa Indonesia)
...
SZRoute adalah gateway routing AI lokal dan dasbor yang dibangun di atas Next.js.
...
- File: `${DATA_DIR}/storage.sqlite` (atau `$XDG_CONFIG_HOME/szroute/storage.sqlite` jika diatur, jika tidak `~/.szroute/storage.sqlite`)
...
- respons klien akhir yang dikembalikan oleh SZRoute; tanggapan yang dialirkan disimpan dalam bentuk ringkasan ringkas yang sama
...
1. `usageDb` dan `localDb` berbagi kebijakan direktori dasar yang sama (`DATA_DIR` -> `XDG_CONFIG_HOME/szroute` -> `~/.szroute`) dengan migrasi file lama.
...
5. Direktori `open-sse/` diterbitkan sebagai `@szroute/open-sse` **paket ruang kerja npm**. Kode sumber mengimpornya melalui `@szroute/open-sse/...` (diselesaikan oleh Next.js `transpilePackages`). Jalur file dalam dokumen ini masih menggunakan nama direktori `open-sse/` untuk konsistensi.
...
- Bangun gambar Docker: `docker build -t szroute .`
- Mulai layanan dan verifikasi:
- `GET /api/settings`
- `GET /api/v1/models`
- URL dasar target CLI harus `http://<host>:21128/v1` ketika `PORT=21128`
