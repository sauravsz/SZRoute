# 🚀 SZRoute — Gateway AI Gratis (Bahasa Indonesia)
...
![NPM Downloads](https://img.shields.io/npm/dw/szroute?label=npm%20down%20week&color=red)
![NPM Downloads](https://img.shields.io/npm/dm/szroute?label=npm%20down%20month&color=red)

![NPM Downloads](https://img.shields.io/npm/d18m/szroute?label=npm%20down%20year&color=red)
![Docker Pulls](https://img.shields.io/docker/pulls/diegosouzapw/szroute)
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/diegosouzapw/szroute/total?style=flat&label=eletron%20donwloads&color=blue)

[![stars](https://custom-icon-badges.demolab.com/github/stars/sauravsz/SZRoute?logo=star&style=flat)](https://github.com/sauravsz/SZRoute/stargazers)
[![open issues](https://custom-icon-badges.demolab.com/github/issues-raw/sauravsz/SZRoute?logo=issue)](https://github.com/sauravsz/SZRoute/issues)
[![license](https://custom-icon-badges.demolab.com/github/license/sauravsz/SZRoute?logo=law)](https://github.com/sauravsz/SZRoute/blob/main/LICENSE)
[![last commit](https://custom-icon-badges.demolab.com/github/last-commit/sauravsz/SZRoute?logo=history&logoColor=white)](https://github.com/sauravsz/SZRoute/commits/main)
...
[![code size](https://custom-icon-badges.demolab.com/github/languages/code-size/sauravsz/SZRoute?logo=file-code&logoColor=white)](https://github.com/sauravsz/SZRoute)
[![pr closed](https://custom-icon-badges.demolab.com/github/issues-pr-closed/sauravsz/SZRoute?color=purple&logo=git-pull-request&logoColor=white)](https://github.com/sauravsz/SZRoute/pulls?q=is%3Apr+is%3Aclosed)
[![tag](https://custom-icon-badges.demolab.com/github/v/tag/sauravsz/SZRoute?logo=tag&logoColor=white)](https://github.com/sauravsz/SZRoute/tags)
...
[![fork](https://custom-icon-badges.demolab.com/github/forks/sauravsz/SZRoute?logo=fork)](https://github.com/sauravsz/SZRoute/network/members)
[![watch](https://custom-icon-badges.demolab.com/github/watchers/sauravsz/SZRoute?logo=eye)](https://github.com/sauravsz/SZRoute/watchers)

[![License](https://img.shields.io/github/license/sauravsz/SZRoute)](https://github.com/sauravsz/SZRoute/blob/main/LICENSE)
...
[🌐 Website](https://szroute.online) • [🚀 Mulai Cepat](#-quick-start) • [💡 Fitur](#-key-features) • [📖 Dokumentasi](#-documentation) • [💰 Harga](#-pricing-at-a-glance) • [💬 WhatsApp](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t)
...
<div align="center">
  <img src="./docs/screenshots/MainSZRoute.png" alt="SZRoute Dashboard" width="800"/>
</div>
...
_Hubungkan IDE atau alat CLI berbasis AI apa pun melalui SZRoute — gateway API gratis untuk coding tanpa batas._
...
<sub>📡 Semua agen terhubung melalui <code>http://localhost:21128/v1</code> atau <code>http://cloud.szroute.online/v1</code> — satu konfigurasi, model dan kuota tak terbatas</sub>
...
## 🎯 Apa yang Diselesaikan SZRoute — 30 Masalah Nyata & Kasus Penggunaan

> **Setiap developer yang menggunakan alat AI menghadapi masalah ini setiap hari.** SZRoute dibangun untuk menyelesaikannya semua — dari pembengkakan biaya hingga pemblokiran regional, dari alur OAuth yang rusak hingga operasi protokol dan observabilitas enterprise.
...
- **Endpoint Terpadu** — Satu `http://localhost:21128/v1` berfungsi sebagai proxy untuk semua 100+ penyedia
...
- **Satu endpoint, semua model** — Konfigurasi `http://localhost:21128/v1` sekali, akses 100+ penyedia
...
**Bagaimana SZRoute menyelesaikannya:**
...
- **Laporan Info Sistem** — `npm run system-info` menghasilkan `system-info.txt` dengan lingkungan lengkap Anda (versi Node, versi SZRoute, OS, alat CLI, status Docker/PM2). Lampirkan saat melaporkan masalah untuk triase instan.
...
- **instal global npm** — `npm install -g szroute && szroute` — selesai
...
- Mulai cepat bawaan untuk `szroute --mcp` dan orientasi klien
...
```txt
1) Start MCP transport (`szroute --mcp`) for tool-driven operations
...
**Arahkan IDE/CLI apa pun ke:** `http://localhost:21128/v1` · Kunci API: `any-string` · Selesai.
...
Dasbor terbuka di `http://localhost:21128` dan URL dasar API adalah `http://localhost:21128/v1`.

#### Arch Linux (AUR)

Pengguna Arch Linux dapat menginstal [AUR package](https://aur.archlinux.org/packages/szroute-bin), yang menginstal SZRoute dan menyediakan layanan pengguna systemd:
...
| Command                 | Description                                                 |
| ----------------------- | ----------------------------------------------------------- |
| `szroute`             | Mulai server (`PORT=21128`, API dan dasbor pada port yang sama) |
| `szroute --port 3000` | Set canonical/API port to 3000                              |
| `szroute --mcp`       | Mulai server MCP (stdio transport)                          |
...
Saat Anda tidak lagi memerlukan SZRoute, kami menyediakan dua skrip cepat untuk penghapusan bersih:

| Command                  | Action                                                                              |
| ------------------------ | ----------------------------------------------------------------------------------- |
| `npm run uninstall`      | Menghapus aplikasi sistem tetapi **menyimpan DB dan konfigurasi Anda** di `~/.szroute`.  |
| `npm run uninstall:full` | Menghapus aplikasi DAN secara permanen **menghapus semua konfigurasi, kunci, dan database**. |

> Catatan: Untuk menjalankan perintah ini, navigasikan ke folder proyek SZRoute (jika Anda mengkloningnya) dan jalankan. Alternatifnya, jika diinstal secara global, Anda cukup menjalankan `npm uninstall -g szroute`.
...
| `STREAM_IDLE_TIMEOUT_MS` | inherits `REQUEST_TIMEOUT_MS` | Kesenjangan maksimum antara potongan streaming sebelum SZRoute membatalkan aliran SSE                                                                  |
...
Untuk upstream yang kompatibel dengan Kode Claude (`anthropic-compatible-cc-*`), SZRoute juga memperoleh header `X-Stainless-Timeout` keluar dari batas waktu pengambilan yang diselesaikan sehingga batas waktu baca sisi penyedia tetap selaras dengan konfigurasi env Anda.

Untuk reverse proxy pihak ketiga yang kompatibel dengan Claude Code, SZRoute tetap menggunakan default
...
`cache_control`, SZRoute tidak memasukkan penanda milik jembatan.
...
Untuk permintaan streaming, `FETCH_TIMEOUT_MS` hanya mencakup pengaturan koneksi/menunggu respons upstream pertama. Setelah aliran aktif, SZRoute hanya akan dibatalkan pada keadaan terhenti sebenarnya (`STREAM_IDLE_TIMEOUT_MS`) atau tubuh Undici tidak aktif (`FETCH_BODY_TIMEOUT_MS`).

Jika Anda menjalankan SZRoute di belakang Nginx, Caddy, Cloudflare, atau proksi terbalik lainnya, pastikan proksi tersebut
waktu tunggu juga lebih tinggi daripada waktu tunggu aliran/pengambilan SZRoute Anda.
...
### 3) Arahkan alat pengkodean Anda ke SZRoute
...
Untuk pengguna Void Linux, Anda dapat membuat paket asli menggunakan `xbps-src`. Simpan blok ini sebagai `srcpkgs/szroute/template`:
...
SZRoute tersedia sebagai image Docker publik di [Docker Hub](https://hub.docker.com/r/diegosouzapw/szroute).
...
- Terowongan Cepat tidak dipulihkan secara otomatis setelah SZRoute atau kontainer dimulai ulang. Aktifkan kembali dari dasbor bila diperlukan.
...
- SQLite berjalan dalam mode WAL. `docker stop` harus dibiarkan selesai sehingga SZRoute dapat memeriksa kembali perubahan terbaru ke `storage.sqlite`.
...
- Setel `CLOUDFLARED_BIN=/absolute/path/to/cloudflared` jika Anda ingin SZRoute menggunakan biner yang sudah ada alih-alih mengunduhnya.

**Menggunakan Docker Compose dengan Caddy (HTTPS Auto-TLS):**

SZRoute dapat diekspos dengan aman menggunakan penyediaan SSL otomatis Caddy. Pastikan data DNS A domain Anda mengarah ke IP server Anda.
...
| `diegosouzapw/szroute` | `3.6.2`  | ~250MB | Current version       |

---

## 🖥️ Aplikasi Desktop — Offline & Selalu Aktif

> 🆕 **BARU!** SZRoute kini tersedia sebagai **aplikasi desktop asli** untuk Windows, macOS, dan Linux.

Jalankan SZRoute sebagai aplikasi desktop mandiri — tanpa terminal, tanpa browser, tanpa internet untuk model lokal. Aplikasi berbasis Electron meliputi:

- 🖥️ **Jendela Asli** — Jendela aplikasi khusus dengan integrasi baki sistem
- 🔄 **Mulai Otomatis** — Luncurkan SZRoute saat login sistem
...
Saat diminimalkan, SZRoute ada di baki sistem Anda dengan tindakan cepat:
...
**Tanpa biaya. Jangan pernah berhenti melakukan pengkodean.** Konfigurasikan ini sebagai satu kombo SZRoute dan semua fallback terjadi secara otomatis — tidak pernah ada peralihan manual.
...
> Semua model di bawah **100% gratis tanpa memerlukan kartu kredit**. SZRoute melakukan rute otomatis di antara keduanya ketika satu kuota habis — gabungkan semuanya untuk kombo $0 yang tidak dapat dipecahkan.
...
SZRoute v3.6 dibangun sebagai platform operasional, bukan hanya proxy relai.
...
Ketika kuota, tarif, atau kesehatan gagal, SZRoute secara otomatis berpindah ke kandidat berikutnya tanpa peralihan manual.
...
SZRoute menyertakan kerangka evaluasi bawaan untuk menguji kualitas respons LLM terhadap rangkaian emas. Akses melalui **Analytics → Evals** di dasbor.

### Golden Set Bawaan

"SZRoute Golden Set" yang dimuat sebelumnya berisi kasus uji untuk:
...
2. Jalankan `szroute_get_health`.
3. Jalankan `szroute_list_combos`.
...
**Kiat Pro:** Gunakan Opus untuk tugas kompleks, Soneta untuk kecepatan. SZRoute melacak kuota per model!
...
- Perilaku rotasi: SZRoute merutekan ke akun Codex berikutnya yang memenuhi syarat secara otomatis.
...
**Kiat Pro:** API yang kompatibel dengan OpenAI — bekerja secara lancar dengan terjemahan format SZRoute!
...
> **Catatan:** OpenClaw hanya berfungsi dengan SZRoute lokal. Gunakan `127.0.0.1` alih-alih `localhost` untuk menghindari masalah resolusi IPv6.
...
Settings → API Configuration:
  ...
  Base URL: http://localhost:21128/v1
  API Key: [from SZRoute dashboard]
  Model: if/kimi-k2-thinking
```

### OpenCode

**Langkah 1:** Tambahkan SZRoute sebagai penyedia khusus:

```bash
opencode
/connect
# Select "Other" → Enter ID: "szroute" → Enter your SZRoute API key
```
...
```bash
/models
# Select any SZRoute model from the list
```

> **Tips:** Tambahkan model apa pun yang tersedia di titik akhir SZRoute `/v1/models` Anda ke bagian `models`. Gunakan format `provider/model-id` dari dasbor SZRoute Anda.
...
- Disegarkan secara otomatis oleh SZRoute
...
- SZRoute v1.0.6+ menyertakan validasi fallback melalui penyelesaian obrolan
...
> **⚠️ Penting bagi pengguna yang menjalankan SZRoute di VPS, Docker, atau server jarak jauh mana pun**
...
Kredensial OAuth yang disertakan dalam SZRoute didaftarkan **hanya untuk `localhost`**. Saat Anda mengakses SZRoute di server jarak jauh (misalnya `https://szroute.myserver.com`), Google menolak autentikasi dengan:
...
- Nama: apa pun yang Anda suka (mis. `SZRoute Remote`)
...
> Ganti `your-server.com` dengan domain atau IP server Anda (sertakan port jika diperlukan, misalnya `http://45.33.32.156:21128/callback`).
...
1. SZRoute membuka URL otorisasi Google
...
4. Tempelkan URL tersebut ke bidang yang ditampilkan di modal koneksi SZRoute
5. Klik **"Hubungkan"**
...
As credenciais OAuth embutidas no SZRoute estão cadastradas **apenas para `localhost`**. Quando você acessa o SZRoute em um servidor remoto (ex: `https://szroute.meuservidor.com`), o Google rejeita a autenticação com:
...
- Nama: nama escolha qualquer (misal: `SZRoute Remote`)
...
> Substitua `seu-servidor.com` pelo domínio ou IP do seu servidor (inclua a porta se necessário, ex: `http://45.33.32.156:21128/callback`).
...
1. O SZRoute abrirá a URL de autorização do Google
2. Após você autorizar, o Google tentará redirecionar para `localhost` (que falha no servidor remoto)
3. **Copie a URL completa** da barra de endereço do seu browser (mesmo que a página não carregue)
4. Cole essa URL no campo que aparece no modal de conexão do SZRoute
5. Clique em **"Connect"**
...
- **Situs Web**: [szroute.online](https://szroute.online)
- **Paket**: [npmjs.com/package/szroute](https://www.npmjs.com/package/szroute)
- **Pekerja Pelabuhan**: [hub.docker.com/r/diegosouzapw/szroute](https://hub.docker.com/r/diegosouzapw/szroute)
...
SZRoute memiliki **218+ fitur yang direncanakan** di berbagai fase pengembangan. Berikut adalah bidang-bidang utamanya:
...
<div align="center">
  ...
  <br/>
  <sub><a href="https://szroute.online">szroute.online</a></sub>
</div>
