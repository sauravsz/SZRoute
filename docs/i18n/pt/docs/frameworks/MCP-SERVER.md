# SZRoute MCP Server Documentation (Português (Portugal))
...
| Tool                            | Description                              |
| :------------------------------ | :--------------------------------------- |
| `szroute_get_health`          | Gateway health, circuit breakers, uptime |
| `szroute_list_combos`         | All configured combos with models        |
| `szroute_get_combo_metrics`   | Performance metrics for a specific combo |
| `szroute_switch_combo`        | Switch active combo by ID/name           |
| `szroute_check_quota`         | Quota status per provider or all         |
| `szroute_route_request`       | Send a chat completion through SZRoute |
| `szroute_cost_report`         | Cost analytics for a time period         |
| `szroute_list_models_catalog` | Full model catalog with capabilities     |

## Advanced Tools (8)

| Tool                               | Description                                                 |
| :--------------------------------- | :---------------------------------------------------------- |
| `szroute_simulate_route`         | Dry-run routing simulation with fallback tree               |
| `szroute_set_budget_guard`       | Session budget with degrade/block/alert actions             |
| `szroute_set_resilience_profile` | Apply conservative/balanced/aggressive preset               |
| `szroute_test_combo`             | Live-test all models in a combo via a real upstream request |
| `szroute_get_provider_metrics`   | Detailed metrics for one provider                           |
| `szroute_best_combo_for_task`    | Task-fitness recommendation with alternatives               |
| `szroute_explain_route`          | Explain a past routing decision                             |
| `szroute_get_session_snapshot`   | Full session state: costs, tokens, errors                   |
