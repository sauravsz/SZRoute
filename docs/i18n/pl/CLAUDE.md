...
npm run dev                    # Serwer deweloperski na http://localhost:21128
...
**SZRoute** — zjednoczony proxy/router AI. Jeden punkt końcowy, 160+ dostawców LLM, automatyczne przełączanie.
...
SZRoute ma trzy powiązane, ale odrębne mechanizmy tymczasowej awarii. Zachowaj ich zakres oddzielnie podczas debugowania zachowania routingu. Zobacz
...
- **Importy**: zewnętrzne → wewnętrzne (`@/`, `@szroute/open-sse`) → względne
...
- **Alias ścieżek**: `@/*` → `src/`, `@szroute/open-sse` → `open-sse/`, `@szroute/open-sse/*` → `open-sse/*`
- **Domyślny port**: 21128 (API + dashboard na tym samym porcie)
- **Katalog danych**: zmienna środowiskowa `DATA_DIR`, domyślnie `~/.szroute/`
