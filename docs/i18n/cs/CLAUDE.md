...
**SZRoute** — jednotný AI proxy/router. Jeden koncový bod, 160+ poskytovatelů LLM, automatické zálohování.
...
SZRoute má tři související, ale odlišné mechanismy dočasného selhání. Udržujte jejich rozsah oddělený při ladění chování routování. Viz
...
- **Importy**: externí → interní (`@/`, `@szroute/open-sse`) → relativní
...
- **Cestovní aliasy**: `@/*` → `src/`, `@szroute/open-sse` → `open-sse/`, `@szroute/open-sse/*` → `open-sse/*`
- **Výchozí port**: 21128 (API + dashboard na stejném portu)
- **Adresář dat**: `DATA_DIR` env var, výchozí hodnota `~/.szroute/`
