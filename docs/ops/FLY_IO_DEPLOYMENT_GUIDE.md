...
本文档记录 SZRoute 在 Fly.io 上的实际部署方法，适用于两类场景：

- 首次把当前项目部署到 Fly.io
- 后续代码更新后继续发布
- 新项目参考同样流程部署

本文基于当前项目已经验证通过的配置整理，应用名为 `szroute`。
...
```toml
app = 'szroute-yourname'
```

注意：

- 控制台里要看的是与 `fly.toml` 里 `app` 一致的应用
- 以前如果用过别的名字，例如 `oroute`，不要和 `szroute` 混淆
...
```powershell
flyctl apps create szroute
```

如果你已经改成别的应用名，把 `szroute` 替换成你的名字。
...
- `SZROUTE_WS_BRIDGE_SECRET` (生产环境必需 / required in production / obrigatório em produção — 用于 WebSocket 桥接鉴权 / used for WebSocket bridge authentication)
...
| `SZROUTE_WS_BRIDGE_SECRET` | 生产必需 (required / obrigatório) | WebSocket 桥接鉴权密钥 (WebSocket bridge auth / chave de autenticação da ponte WebSocket) |
...
| ---------------------- | --------------------------- |
| `DATA_DIR`             | `/data`                     |
| `NEXT_PUBLIC_BASE_URL` | `https://szroute.fly.dev` |
...
1. **设置 `NEXT_PUBLIC_BASE_URL` 指向你公开的 HTTPS 域名 (set `NEXT_PUBLIC_BASE_URL` to the public HTTPS domain / defina `NEXT_PUBLIC_BASE_URL` para o domínio HTTPS público)**
   ...
   ```powershell
   flyctl secrets set NEXT_PUBLIC_BASE_URL=https://szroute.fly.dev -a szroute
   ```

   如果你使用了自定义域名 (if using a custom domain / se usar um domínio personalizado)，请替换为对应域名 (e.g. `https://szroute.yourdomain.com`)。
...
2. **在 provider 控制台配置回调 URL (configure the callback URL on the provider console / configure a URL de callback no painel do provider)**
   ...
   例如 (e.g. / p.ex.)：
   - `https://szroute.fly.dev/api/oauth/gemini/callback`
   - `https://szroute.fly.dev/api/oauth/antigravity/callback`
   - `https://szroute.fly.dev/api/oauth/cursor/callback`
...
flyctl secrets set `
  ...
  SZROUTE_WS_BRIDGE_SECRET=$wsBridgeSecret `
  DATA_DIR=/data `
  NEXT_PUBLIC_BASE_URL=https://szroute.fly.dev `
  -a szroute
```
...
```bash
flyctl secrets set SZROUTE_WS_BRIDGE_SECRET=$(openssl rand -hex 32) -a szroute
```

说明 (notes / observações)：

- `SZROUTE_WS_BRIDGE_SECRET` 在生产环境必需，缺失会导致 WebSocket 桥接握手失败 (required in production; missing it breaks WebSocket bridge handshake / obrigatório em produção; sem ele o handshake da ponte WebSocket falha)

如果你还要加初始密码：

```powershell
flyctl secrets set INITIAL_PASSWORD=你的强密码 -a szroute
```
...
```powershell
flyctl secrets set KEY=value -a szroute
```
...
如果当前仓库是 fork，并且你要同步上游 `https://github.com/sauravsz/SZRoute` 的更新，推荐按下面流程执行。
...
5. `flyctl deploy`
6. `flyctl status -a szroute`
7. `flyctl logs --no-tail -a szroute`
...
```powershell
flyctl status -a szroute
```

### 10.2 查看启动日志

```powershell
flyctl logs --no-tail -a szroute
```
...
```powershell
try {
  (Invoke-WebRequest -Uri "https://szroute.fly.dev" -MaximumRedirection 5 -UseBasicParsing).StatusCode
} catch {
...
- 你打开的是另一个应用，例如 `oroute`，不是 `szroute`
...
1. `flyctl auth login`
2. `flyctl apps create szroute`
3. `flyctl secrets set ... -a szroute`
4. `flyctl deploy`
5. `flyctl logs --no-tail -a szroute`
