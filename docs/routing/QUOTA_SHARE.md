...
**Problem it solves:** SZRoute proxies many API keys against the same upstream
...
- All persistence is in the existing SZRoute SQLite DB (`DATA_DIR/storage.sqlite`).
...
1. `localStorage.getItem("szroute:quota-share:pools")` is non-empty.
