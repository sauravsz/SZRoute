...
This guide covers adding Kiro (AWS-hosted AI coding assistant) accounts to SZRoute,
...
## How SZRoute Solves This (v3.8.0+)

Starting with v3.8.0, SZRoute calls `registerClient()` (AWS SSO OIDC) during every
Kiro connection import. This gives each SZRoute connection its own dedicated OIDC
...
2. Choose one of:
   ...
   - **Auto-Import** — click the button; SZRoute reads credentials from the
     local kiro-cli database or `~/.aws/sso/cache`.
3. The connection is saved. SZRoute automatically registers a dedicated OIDC client for it.
...
```bash
curl -X POST http://localhost:21128/api/oauth/kiro/import \
...
(`clientSecretExpiresAt`). SZRoute stores this timestamp in `providerSpecificData`
...
- Ensure SZRoute can reach `https://oidc.us-east-1.amazonaws.com` (or the configured
