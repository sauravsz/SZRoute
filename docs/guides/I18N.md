...
SZRoute supports **30 languages** with full dashboard UI translation, translated documentation, and RTL support for Arabic and Hebrew.
...
SZRoute uses a hash-based incremental translator for docs, backed by an
OpenAI-compatible LLM endpoint (typically `cx/gpt-5.4-mini` through SZRoute
Cloud):
...
| Variable                            | Purpose                                 |
| ----------------------------------- | --------------------------------------- |
| `SZROUTE_TRANSLATION_API_URL`     | OpenAI-compatible base URL, e.g. `…/v1` |
| `SZROUTE_TRANSLATION_API_KEY`     | bearer token (kept out of logs)         |
| `SZROUTE_TRANSLATION_MODEL`       | model id, e.g. `cx/gpt-5.4-mini`        |
...
- **Chunked batching**: Joins multiple strings with `__SZROUTE_I18N_SEPARATOR__` delimiters to minimize API calls (max 1800 chars per request)
...
**Secondary translator** — uses any OpenAI-compatible LLM API (including SZRoute itself) to translate existing `docs/i18n/` markdown files. Best for polishing or re-translating docs with better quality than Google Translate.

```bash
python3 scripts/i18n/i18n_autotranslate.py \
  --api-url http://localhost:21128/v1 \
  --api-key sk-your-key \
  --model gpt-4o
```
...
The `szroute` CLI has its own i18n layer separate from the Next.js dashboard.
...
| -------- | ------------------------ | --------------------------------------- |
| 1        | `--lang` flag            | `szroute --lang de status`            |
| 2        | `SZROUTE_LANG` env var | `SZROUTE_LANG=ja szroute providers` |
...
```bash
# Set language and save to ~/.szroute/.env (persists across sessions)
szroute config lang set pt-BR

# View current language
szroute config lang get

# List all 42 available languages
szroute config lang list

# JSON output
szroute config lang list --output json
```

The saved preference is written atomically to `~/.szroute/.env` and is loaded by the
...
szroute --lang de providers list
```
...
```bash
# Default: es, fr, de, ja, ar on localhost:21128
