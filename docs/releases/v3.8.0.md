---
title: "SZRoute v3.8.0 Release Notes"
version: 3.8.0
releaseDate: 2026-05-15
---

# SZRoute v3.8.0 — Release Notes
...
Ten `SKILL.md` manifests under `skills/szroute*/` published for external AI
...
`systray2` fork installed lazily into `~/.szroute/runtime/`. Includes
cross-platform autostart (LaunchAgent / `.desktop` / registry).

- New flag: `szroute --tray`
- New command: `szroute config tray <enable|disable>`
- New modules: `bin/cli/tray/`

### CLI machine-ID token (Task 4)

`HMAC-SHA256(machine-id, salt)` → constant-time-compared token sent via
`x-szroute-cli-token` header. Lets CLI commands authenticate locally
without JWT/password.

- Accepted only on loopback host + whitelisted read-only routes
- Salt rotation via `SZROUTE_CLI_SALT` env var
- Falls back to `~/.szroute/machine-id` (0600) if `node-machine-id` fails
...
load. Solves Windows EBUSY error on global updates while SZRoute is running.

- New scripts: `scripts/postinstall.mjs`, `bin/cli/runtime/`
- Runtime dir: `~/.szroute/runtime/`
- New helper: `src/lib/db/core.ts::ensureDbInitialized()`
- Docs: `docs/ops/SQLITE_RUNTIME.md`

### Tier 1/2/3 marketing & onboarding (Task 8)

README "Why SZRoute?" section enhanced with ASCII tier diagram and comparison
...
```bash
npm install -g szroute@3.8.0
szroute setup   # optional — re-runs onboarding to show new tier tour
```

CLI tokens are derived automatically; no migration needed.
To enable tray: `szroute --tray` or `szroute config tray enable`.

For developers embedding SZRoute:
