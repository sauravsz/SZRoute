---
title: "SZRoute Codebase Documentation"
version: 3.8.2
lastUpdated: 2026-05-13
---

# SZRoute Codebase Documentation

> **Version:** v3.8.0
> **Last updated:** 2026-05-13
> **Audience:** Engineers contributing to SZRoute or building integrations on top of it.
>
...
- `@/*` → `src/*`
- `@szroute/open-sse` → `open-sse/index.ts`
- `@szroute/open-sse/*` → `open-sse/*`

Default HTTP port: **`21128`** (API and dashboard share the same process). Data
directory is `DATA_DIR` env var, defaulting to `~/.szroute/`.
...
├── open-sse/             Streaming engine workspace (@szroute/open-sse)
├── electron/             Desktop wrapper (Electron 41 main + preload)
├── bin/                  CLI entry points (szroute, reset-password)
...
| `responses.ts`, `szrouteResponseMeta.ts` | Response shape helpers                            |
...
Separate npm workspace published as `@szroute/open-sse`. Owns request
...
```
bin/
├── szroute.mjs           Main CLI entry (Node ESM)
...
- `szroute` → `bin/szroute.mjs`
- `szroute-reset-password` → `bin/reset-password.mjs`
