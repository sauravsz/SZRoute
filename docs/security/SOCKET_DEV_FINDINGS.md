...
description: "Maintainer attestation for the AI-detected potential-malware findings raised against szroute and the v3.8.6 mitigations applied at each flagged call site."
---

# Socket.dev / supply-chain finding attestation

This document is the maintainer-authored attestation for the six
`AI-detected potential malware` findings raised against `szroute@3.8.5` and
the mitigations applied in `szroute@3.8.6`. It exists so:
...
Caddy. The fact that they exist in SZRoute is documented at
...
A `SZROUTE_ZED_IMPORT_LEGACY_ONE_STEP=true` env flag preserves the v3.8.5
...
who already use Zed and want to mirror their provider keys into SZRoute
without re-pasting.
...
toolkit." Within SZRoute they are only used by the MITM cert install path
...
(`SZROUTE_BUILD_PROFILE=minimal`) replaces
...
## §5 — SZRoute Cloud Sync credential write-back (`api/keys/[id]/route.js`)
...
1. **HMAC verification**: `verifyCloudSignature(rawBody, sigHeader)` checks
   the `X-Cloud-Sig` header (`HMAC-SHA256(SZROUTE_CLOUD_SYNC_SECRET,
...
2. **Secret-field opt-in**: `accessToken` / `refreshToken` /
   ...
   `SZROUTE_CLOUD_SYNC_SECRETS=true`. The default mode syncs only
...
**Why we keep it**: Cloud Sync is the only way for an SZRoute Cloud tenant
...
```bash
SZROUTE_BUILD_PROFILE=minimal npm run build
```
...
The resulting bundle is intended to be published as `szroute-secure`. See
