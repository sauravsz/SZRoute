...
SZRoute includes a local-first gamification layer that rewards users for
...
| Community Servers | Federate with external SZRoute instances                      |
...
All tables live in the main SZRoute SQLite database, created by migration
...
Follows the standard SZRoute pattern — imports `getDbInstance()` from
...
The raw token is returned to the user exactly once at creation time. SZRoute
...
All routes follow the standard SZRoute pattern:
