...
AgentBridge is SZRoute's MITM (Man-in-the-Middle) proxy that intercepts HTTPS traffic from IDE AI agents and reroutes it through SZRoute's unified routing engine. It supports **9 IDE agents** — Antigravity, Kiro, GitHub Copilot, OpenAI Codex, Cursor, Zed, Claude Code, Open Code, and Trae (investigating) — making SZRoute the broadest-coverage MITM proxy for AI coding assistants on the market.
...
- **Reroute any agent to any provider**: Copilot talking to OpenAI? Redirect it to Anthropic Claude, Gemini, or any of SZRoute's 160+ providers.
...
- **Apply SZRoute resilience**: combo routing, circuit breakers, fallbacks, and cost tracking work for IDE agent traffic too.

### Positioning vs. the market

| Feature | 9router | anti-api | llm-interceptor | **SZRoute AgentBridge** |
|---------|:-------:|:--------:|:---------------:|:-------------------------:|
| Antigravity | ✓ | ✓ | — | ✓ |
| GitHub Copilot | ✓ | ✓ | — | ✓ |
| Kiro (AWS) | ✓ | ✓ | — | ✓ |
...
src/mitm/server.cjs  (port 443, CJS child process)
    ...
    ├── Target match? → fetch → SZRoute router (port 21128)
...
- Dispatches to the TypeScript handler layer via HTTP to `http://127.0.0.1:21128`
...
```bash
certutil -A -d sql:$HOME/.pki/nssdb -n "SZRoute AgentBridge" -t CT,, -i ~/.szroute/mitm/ca.crt
```

**macOS (Keychain):**
```bash
sudo security add-trusted-cert -d -r trustRoot \
  -k /Library/Keychains/System.keychain ~/.szroute/mitm/ca.crt
```

**Windows (certmgr):**
```powershell
certutil -addstore -f Root $env:USERPROFILE\.szroute\mitm\ca.crt
```
...
| Source model (agent native) | Target model (SZRoute) |
|-----------------------------|--------------------------|
...
AgentBridge intercepts credentials (OAuth tokens, API keys) that the IDE uses to authenticate with upstream providers. These are **masked before logging** (see §2.7) but are visible to SZRoute's MITM layer. First activation of each agent shows a dismissible risk notice modal.
...
The bypass list ensures that financial institutions, OAuth/SSO providers, and other sensitive hosts are **never decrypted**. Their TLS traffic passes through as a transparent TCP tunnel — SZRoute never sees the plaintext.
...
1. Verify the cert was installed: `security find-certificate -c "SZRoute AgentBridge"` (macOS) or `certutil -L -d sql:$HOME/.pki/nssdb` (Linux/NSS)
...
```bash
grep "szroute\|127.0.0.1.*github\|127.0.0.1.*cursor" /etc/hosts
```
...
2. Check SZRoute server logs: `APP_LOG_LEVEL=debug` in `.env`
3. Verify `SZROUTE_BASE_URL` points to the correct router endpoint (default: `http://127.0.0.1:21128`)
