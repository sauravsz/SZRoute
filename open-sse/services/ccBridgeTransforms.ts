 ...
 *
 * Goal: ensure the final request body SZRoute sends to Anthropic's
...
export interface InjectBillingHeaderOp {
   ...
   *   - szroute-daystamp: sha256(YYYY-MM-DD + version).slice(0,3)
   */
  versionFormat: "ex-machina" | "szroute-daystamp";
  /**
 ...
 * proven against the live SZRoute deployment (call log
...
/**
 * Compute the `cc_version` suffix per the SZRoute native-OAuth algorithm:
...
export function buildBillingHeaderValue(
  ...
  const suffix =
    options.versionFormat === "szroute-daystamp"
