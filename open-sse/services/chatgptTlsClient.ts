...
const DEFAULT_TIMEOUT_MS =
  Number.parseInt(process.env.SZROUTE_CHATGPT_TLS_TIMEOUT_MS || "", 10) || 60_000;
...
const HARD_TIMEOUT_GRACE_MS =
  Number.parseInt(process.env.SZROUTE_CHATGPT_TLS_GRACE_MS || "", 10) || 10_000;
  ...
  if (!clientPromise) {
    clientPromise = (async () => {
      try {
        ...
        // managed sidecar's localhost HTTP calls that SZRoute's global fetch
...
export interface TlsFetchOptions {
   ...
   *   2. `process.env.SZROUTE_TLS_PROXY_URL` (single-flag opt-in)
