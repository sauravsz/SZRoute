...
> **Model Context Protocol server** that exposes SZRoute's gateway intelligence as **37 tools** for AI agents.
>
...
The MCP Server allows any AI agent (Claude Desktop, Cursor, VS Code Copilot, custom agents) to **monitor, control, and optimize** the SZRoute AI gateway programmatically.
...
```bash
# Required: SZRoute base URL
export SZROUTE_BASE_URL="http://localhost:21128"

# Optional: API key for authenticated access
export SZROUTE_API_KEY="your-api-key"

# Optional: Scope enforcement (default: disabled)
export SZROUTE_MCP_ENFORCE_SCOPES="true"
export SZROUTE_MCP_SCOPES="read:health,read:combos,read:quota,read:usage,read:models,read:cache,read:compression,execute:completions,write:combos,write:budget,write:resilience,write:cache,write:compression"
```
...
```json
{
  "mcpServers": {
    "szroute": {
      "command": "node",
      "args": ["path/to/szroute/open-sse/mcp-server/server.ts"],
      "env": {
        "SZROUTE_BASE_URL": "http://localhost:21128",
        "SZROUTE_API_KEY": "your-key"
      }
    }
  }
}
...
| --- | ------------------------------- | --------------------- | -------------------------------------------------------------------------- |
| 1   | `szroute_get_health`          | `read:health`         | Gateway health, uptime, memory, circuit breakers, rate limits, cache stats |
| 2   | `szroute_list_combos`         | `read:combos`         | List all combos (model chains) with strategies and optional metrics        |
| 3   | `szroute_get_combo_metrics`   | `read:combos`         | Performance metrics for a specific combo                                   |
| 4   | `szroute_switch_combo`        | `write:combos`        | Activate or deactivate a combo for routing                                 |
| 5   | `szroute_check_quota`         | `read:quota`          | Remaining API quota per provider with token health status                  |
| 6   | `szroute_route_request`       | `execute:completions` | Send a chat completion through intelligent routing                         |
| 7   | `szroute_cost_report`         | `read:usage`          | Cost report by period (session/day/week/month) with per-provider breakdown |
| 8   | `szroute_list_models_catalog` | `read:models`         | List all available models across providers with capabilities and pricing   |

### Phase 2: Advanced Tools (8)

| #   | Tool                               | Scopes                               | Description                                                                                    |
| --- | ---------------------------------- | ------------------------------------ | ---------------------------------------------------------------------------------------------- |
| 9   | `szroute_simulate_route`         | `read:health`, `read:combos`         | Dry-run routing simulation showing fallback tree and estimated costs                           |
| 10  | `szroute_set_budget_guard`       | `write:budget`                       | Set session budget with action on exceed: `degrade`, `block`, or `alert`                       |
| 11  | `szroute_set_resilience_profile` | `write:resilience`                   | Apply resilience profile: `aggressive`, `balanced`, or `conservative`                          |
| 12  | `szroute_test_combo`             | `execute:completions`, `read:combos` | Test each provider in a combo with a real prompt and a real upstream call, report latency/cost |
| 13  | `szroute_get_provider_metrics`   | `read:health`                        | Per-provider metrics with latency percentiles (p50/p95/p99), circuit breaker                   |
| 14  | `szroute_best_combo_for_task`    | `read:combos`, `read:health`         | AI-powered combo recommendation by task type with budget/latency constraints                   |
| 15  | `szroute_explain_route`          | `read:health`, `read:usage`          | Explain why a request was routed to a provider (scoring factors, fallbacks)                    |
| 16  | `szroute_get_session_snapshot`   | `read:usage`                         | Full session snapshot: cost, tokens, top models, errors, budget status                         |

### Cache and Compression Tools

| #   | Tool                                | Scopes              | Description                                                                  |
| --- | ----------------------------------- | ------------------- | ---------------------------------------------------------------------------- |
| 21  | `szroute_cache_stats`             | `read:cache`        | Semantic cache, prompt-cache, and idempotency statistics                     |
| 22  | `szroute_cache_flush`             | `write:cache`       | Flush cache entries globally or by signature/model                           |
| 23  | `szroute_compression_status`      | `read:compression`  | Compression settings, analytics summary, and provider-aware cache statistics |
| 24  | `szroute_compression_configure`   | `write:compression` | Configure compression mode and trigger thresholds at runtime                 |
| 25  | `szroute_set_compression_engine`  | `write:compression` | Set Caveman, RTK, or stacked compression mode and pipeline                   |
| 26  | `szroute_list_compression_combos` | `read:compression`  | List named compression combos and routing assignments                        |
| 27  | `szroute_compression_combo_stats` | `read:compression`  | Read analytics grouped by compression combo and engine                       |

MCP listable metadata descriptions are compressed at registration/list time when description
compression is enabled. `szroute_compression_status` exposes those savings separately as
...
```python
"""
SZRoute MCP Client — Python example using the mcp SDK.
...
async def main():
    ...
    async with stdio_client(server) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            health = await session.call_tool("szroute_get_health", {})
            print("Health:", health.content[0].text)

            # 2. List available combos with metrics
            combos = await session.call_tool("szroute_list_combos", {
                "includeMetrics": True
            })
            print("Combos:", combos.content[0].text)

            # 3. Find the best combo for a coding task
            best = await session.call_tool("szroute_best_combo_for_task", {
                "taskType": "coding",
            ...
            budget = await session.call_tool("szroute_set_budget_guard", {
                "maxCost": 1.00,
                "action": "degrade",
                "degradeToTier": "cheap",
            })
            ...
            response = await session.call_tool("szroute_route_request", {
            ...
            snapshot = await session.call_tool("szroute_get_session_snapshot", {})
...
An agent that monitors SZRoute health and auto-switches combos when providers degrade.
...
async def auto_healing_loop(session):
    ...
    while True:
        ...
        if broken:
            ...
            await session.call_tool("szroute_set_resilience_profile", {
            ...
            # Activate it
            await session.call_tool("szroute_switch_combo", {
...
async def budget_aware_coding(session, task: str, max_budget: float):
    ...
    # Set budget guard
    await session.call_tool("szroute_set_budget_guard", {
        "maxCost": max_budget,
        "action": "degrade",
        "degradeToTier": "cheap",
    })
    ...
    sim = await session.call_tool("szroute_simulate_route", {
        "model": "claude-sonnet-4",
    ...
    # Send request
    result = await session.call_tool("szroute_route_request", {
        "model": "claude-sonnet-4",
...
async def benchmark_combos(session):
    ...
    for combo in combo_list:
        ...
        test = await session.call_tool("szroute_test_combo", {
            "comboId": combo["id"],
...
async def find_cheapest_models(session, capability="chat"):
    ...
    catalog = await session.call_tool("szroute_list_models_catalog", {
