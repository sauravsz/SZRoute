...
const FALLBACK_KIE_CALLBACK_URL = "https://szroute.local/api/kie/callback";
...
function getConfiguredKieCallbackUrl(): string {
  const explicit =
    process.env.KIE_CALLBACK_URL?.trim() || process.env.SZROUTE_KIE_CALLBACK_URL?.trim();
  if (explicit) return explicit;

  return (
    callbackUrlFromBaseUrl(process.env.SZROUTE_PUBLIC_URL) ||
