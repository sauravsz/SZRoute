...
import { buildSZRouteSseMetadataComment } from "@/domain/szrouteResponseMeta";
import {
...
export const PENDING_REQUEST_CLEARED_MARKER = "__szroutePendingRequestCleared";
...
export function createSSEStream(options: StreamOptions = {}) {
  ...
  const ensureVisibleResponsesReasoningSummary = (payload: Record<string, unknown>): boolean => {
    ...
    item.summary = [
      {
        type: "summary_text",
        text: "Codex is reasoning, but the upstream Responses API exposed this reasoning block only as encrypted state. SZRoute cannot recover the private reasoning text.",
      },
    ];
    return true;
  };
...
`;
                  ...
                  // chunk like "[SZRoute] Upstream returned an empty response. Please
