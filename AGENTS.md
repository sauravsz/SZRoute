...
- **Database**: better-sqlite3 (SQLite) — `DATA_DIR` configurable, default `~/.szroute/`
...
`/usr/lib/node_modules/szroute/app/` directory (VPS image path is unchanged).
...
- **Order**: external → internal (`@/`, `@szroute/open-sse`) → relative (`./`, `../`)
...
- **`core.ts`**: `getDbInstance()` returns a singleton `better-sqlite3` instance with WAL
  journaling. `SCHEMA_SQL` defines **17 base tables** (verify with `grep -c "CREATE TABLE" src/lib/db/core.ts` minus 1 for the bookkeeping `_szroute_migrations` table). Helpers: `rowToCamel`, `encryptConnectionFields`.
- **`migrationRunner.ts`**: Applies versioned SQL files from `db/migrations/` inside transactions.
  Tracks applied migrations in `_szroute_migrations` table.
...
- **Transports**: stdio (CLI `szroute --mcp`), SSE (`/api/mcp/sse`), Streamable HTTP
...
- **Built-in skills**: Ship with SZRoute (e.g., quota management, routing). Located
...
This repository is a fork of `sauravsz/SZRoute`. Keep fork-only operational
