 ...
 *  2. Zero szroute-* folders remain (post-prune: old szroute-* skill dirs were removed).
...
// ── §2: No szroute-* directories remain ────────────────────────────────────

test("skills/ has zero szroute-* directories (all pruned)", () => {
  ...
  const omniRouteDirs = entries
    .filter((e) => e.isDirectory() && e.name.startsWith("szroute-"))
    .map((e) => e.name);
  assert.deepEqual(
    omniRouteDirs,
    [],
    `Found szroute-* directories that should have been pruned: ${omniRouteDirs.join(", ")}`,
  );
});
