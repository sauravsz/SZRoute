 ...
 *  3. MCP tool szroute_agent_skills_list handler returns 43 entries.
...
// ── §3: MCP tool szroute_agent_skills_list ─────────────────────────────────

test("MCP szroute_agent_skills_list handler returns count 43", async () => {
...
test("MCP szroute_agent_skills_list result has all 42 IDs", async () => {
  const result = await agentSkillTools.szroute_agent_skills_list.handler({});
