...
test("discoverPlugins retorna array vazio se nenhum plugin instalado (diretório não existe)", async () => {
  ...
  const orig = process.env.SZROUTE_PLUGIN_PATH;
  process.env.SZROUTE_PLUGIN_PATH = join(tmpdir(), `no-such-dir-${Date.now()}`);
  try {
    const plugins = await discoverPlugins();
    assert.ok(Array.isArray(plugins));
  } finally {
    if (orig === undefined) delete process.env.SZROUTE_PLUGIN_PATH;
    else process.env.SZROUTE_PLUGIN_PATH = orig;
  }
});

test("discoverPlugins descobre plugin com package.json válido", async () => {
  const { discoverPlugins } = await import("../../bin/cli/plugins.mjs");
  const pluginDir = join(tmpdir(), `szroute-plugins-test-${Date.now()}`);
  const pkgDir = join(pluginDir, "szroute-cmd-test-hello");
  mkdirSync(pkgDir, { recursive: true });
  writeFileSync(
    join(pkgDir, "package.json"),
    JSON.stringify({
      name: "szroute-cmd-test-hello",
      version: "1.0.0",
      type: "module",
      main: "index.mjs",
    })
  ...
  const orig = process.env.SZROUTE_PLUGIN_PATH;
  process.env.SZROUTE_PLUGIN_PATH = pluginDir;
  try {
    const plugins = await discoverPlugins();
    assert.ok(
      plugins.some((p) => p.name === "szroute-cmd-test-hello"),
...
test("discoverPlugins ignora pacotes sem prefixo szroute-cmd-", async () => {
  const { discoverPlugins } = await import("../../bin/cli/plugins.mjs");
  const pluginDir = join(tmpdir(), `szroute-plugins-test-${Date.now()}`);
...
test("loadPlugins não quebra CLI quando plugin tem erro de load (try/catch)", async () => {
  ...
  const pluginDir = join(tmpdir(), `szroute-plugins-test-${Date.now()}`);
  const pkgDir = join(pluginDir, "szroute-cmd-broken");
  mkdirSync(pkgDir, { recursive: true });
  writeFileSync(
    join(pkgDir, "package.json"),
    JSON.stringify({
      name: "szroute-cmd-broken",
      version: "1.0.0",
      type: "module",
      main: "broken.mjs",
    })
...
test("loadPlugins carrega plugin válido e chama register()", async () => {
  const { loadPlugins } = await import("../../bin/cli/plugins.mjs");
  const pluginDir = join(tmpdir(), `szroute-plugins-test-${Date.now()}`);
  const pkgDir = join(pluginDir, "szroute-cmd-valid");
  mkdirSync(pkgDir, { recursive: true });
  writeFileSync(
    join(pkgDir, "package.json"),
    JSON.stringify({
      name: "szroute-cmd-valid",
      version: "1.0.0",
      type: "module",
      main: "index.mjs",
    })
...
test("exemplo szroute-cmd-hello existe e tem register()", () => {
  const examplePath = join(ROOT, "examples", "szroute-cmd-hello", "index.mjs");
...
test("docs/dev/plugins.md existe", () => {
  ...
  assert.ok(src.includes("szroute-cmd"), "deve mencionar szroute-cmd");
