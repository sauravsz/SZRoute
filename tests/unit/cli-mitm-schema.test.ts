...
// Regression test: null apiKey + unresolvable keyId must yield the sentinel 'sk_szroute',
...
test("sentinel guard condition catches sk_szroute and null", () => {
  const SENTINEL = "sk_szroute";
  // Simulate what the route guard checks: (!apiKey || apiKey === 'sk_szroute')
  ...
  assert.equal(shouldReject("sk_szroute"), true, "sentinel must be rejected");
