 ...
 * (`serve --tray`) or the Electron Appearance tab; a plain `szroute serve`
 * user had no way to enable it. The `autostart` command now exposes the
 * shorthand the reporter asked for (`szroute autostart on` / `... true`) via
...
test("status is the default action (bare `szroute autostart` is a safe read-only)", () => {
