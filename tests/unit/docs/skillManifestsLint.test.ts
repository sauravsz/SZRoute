...
// coletava tests/unit/docs/) e apodreceu — filtrava dirs `szroute*`, mas os skills
...
// invariante de uso é "referencia as env vars ($SZROUTE_URL/SZROUTE_KEY) OU
// comandos da CLI (`szroute …`)" — 3 skills (health/keys/batches) usam só a CLI.
...
test("each skill dir has SKILL.md with frontmatter", async () => {
  ...
  for (const dir of dirs) {
    ...
    if (!dir.startsWith("omni-")) {
      assert.ok(
        content.includes("SZROUTE_") || content.includes("szroute "),
        `${dir}: missing usage references (SZROUTE_* env vars or szroute CLI commands)`
      );
    }
  }
});
