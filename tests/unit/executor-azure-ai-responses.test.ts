...
import { DefaultExecutor } from "@szroute/open-sse/executors/default.ts";
...
test("DefaultExecutor.buildUrl forces /responses path when _szrouteForceResponsesUpstream=true", () => {
  const executor = new DefaultExecutor("azure-ai");

  const forced = executor.buildUrl("gpt-4.1", true, 0, {
    providerSpecificData: {
      baseUrl: "https://example-resource.services.ai.azure.com/openai/v1",
      _szrouteForceResponsesUpstream: true,
    },
  });
