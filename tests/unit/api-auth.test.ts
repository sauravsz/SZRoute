...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "szroute-api-auth-"));
...
test("getApiKeyMetadata recognizes SZROUTE_API_KEY environment variable", async () => {
  const envKey = "sk-test-env-key-" + Date.now();
  process.env.SZROUTE_API_KEY = envKey;
  ...
  delete process.env.SZROUTE_API_KEY;
});
...
function managementCliRequest(token: string) {
  const request = new Request("http://localhost:21128/api/combos", {
...
test("requireManagementAuth returns null for SZROUTE_API_KEY env passthrough", async () => {
  await setupAuth();
