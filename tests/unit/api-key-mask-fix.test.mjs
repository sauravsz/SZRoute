...
describe("resolveApiKey", () => {
  ...
  it("falls back to sk_szroute when both are null", async () => {
    mockKeyStore.clear();

    const result = await resolveApiKey(null, null);
    assert.equal(result, "sk_szroute");
  });

  it("falls back to sk_szroute when both are undefined", async () => {
    mockKeyStore.clear();
