...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "szroute-settings-ext-"));
...
function cleanupGlobalDb() {
  try {
    if ((globalThis as any).__szrouteDb?.open) {
      (globalThis as any).__szrouteDb.close();
    }
  } catch {}
  delete (globalThis as any).__szrouteDb;
}
