...
// #3578 — `szroute --mcp` crashed on npm installs with ERR_MODULE_NOT_FOUND for
...
function resolveImport(fromFile: string, spec: string): string | null {
  ...
  else if (spec.startsWith("@szroute/open-sse/"))
    base = path.join("open-sse", spec.slice("@szroute/open-sse/".length));
  else if (spec === "@szroute/open-sse") base = path.join("open-sse", "index");
