...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "szroute-a2a-enabled-route-"));
const ORIGINAL_DATA_DIR = process.env.DATA_DIR;
const ORIGINAL_SZROUTE_API_KEY = process.env.SZROUTE_API_KEY;
...
test.after(() => {
  ...
  if (ORIGINAL_SZROUTE_API_KEY === undefined) {
    delete process.env.SZROUTE_API_KEY;
  } else {
    process.env.SZROUTE_API_KEY = ORIGINAL_SZROUTE_API_KEY;
  }
});
...
test("A2A JSON-RPC checks auth before returning disabled state", async () => {
  process.env.SZROUTE_API_KEY = "test-secret";
