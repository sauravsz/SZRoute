...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "szroute-installer-"));
const FAKE_BIN_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "szroute-fake-bin-"));
...
test("install creates package.json structure", async () => {
  ...
  assert.equal(parsedHost.name, "szroute-9router-host");
