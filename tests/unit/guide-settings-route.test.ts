...
const DUMMY_HOME = path.join(os.tmpdir(), "szroute-qwen-test-" + Date.now());
...
test("guide-settings POST creates new hermes config.yaml if it doesn't exist", async () => {
  ...
  assert.equal(content.model?.provider, "szroute");
  assert.equal(content.model?.base_url, "http://my-omni/v1");
  assert.equal(content.providers?.szroute?.base_url, "http://my-omni/v1");
  assert.ok(String(content.providers?.szroute?.api_key || "").startsWith("sk-"));
});
...
test("guide-settings POST writes OpenCode config with current schema and multi-model selection", async () => {
  ...
  assert.equal(content.provider.szroute.npm, "@ai-sdk/openai-compatible");
  assert.equal(content.provider.szroute.options.baseURL, "http://my-omni/v1");
  assert.ok(content.provider.szroute.options.apiKey.startsWith("sk-"));
  assert.deepEqual(Object.keys(content.provider.szroute.models), [
...
test("guide-settings POST preserves existing OpenCode config fields while only updating provider.szroute", async () => {
  await fs.mkdir(path.dirname(OPENCODE_CONFIG_PATH), { recursive: true });
  await fs.writeFile(
    OPENCODE_CONFIG_PATH,
    `{
  ...
  assert.equal(content.provider.szroute.options.baseURL, "http://my-omni/v1");
  assert.ok(content.provider.szroute.options.apiKey.startsWith("sk-"));
  assert.deepEqual(content.provider.szroute.models, {
