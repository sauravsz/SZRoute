...
const argv = (...args: string[]) => ["node", "szroute", ...args];
...
test("storage key: bare `szroute` provisions (serve is the default command)", () => {
...
test("storage key: real commands DO provision (preserves #1622 persistence)", () => {
  ...
  assert.equal(shouldProvisionStorageKey(argv("serve", "--port", "21128")), true);
