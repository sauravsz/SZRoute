...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "szroute-lang-test-"));
  origDataDir = process.env.DATA_DIR;
  origOmniLang = process.env.SZROUTE_LANG;
  process.env.DATA_DIR = tmpDir;
  delete process.env.SZROUTE_LANG;
});

test.after(() => {
  if (origDataDir === undefined) delete process.env.DATA_DIR;
  else process.env.DATA_DIR = origDataDir;
  if (origOmniLang === undefined) delete process.env.SZROUTE_LANG;
  else process.env.SZROUTE_LANG = origOmniLang;
  try {
...
test("runConfigLangSetCommand salva locale no .env e chama setLocale imediatamente", async () => {
  ...
  try {
    ...
    assert.ok(content.includes("SZROUTE_LANG=pt-BR"), "Deve persistir SZROUTE_LANG=pt-BR");
...
test("runConfigLangSetCommand --force salva mesmo quando locale já ativo", async () => {
  ...
  try {
    ...
    if (existsSync(envPath)) {
      const content = readFileSync(envPath, "utf8");
      assert.ok(content.includes("SZROUTE_LANG=en"), "Deve ter gravado SZROUTE_LANG=en");
    }
  } finally {
    console.log = origLog;
    resetForTests();
...
test("runConfigLangSetCommand atualiza chave sem duplicar quando já existe", async () => {
  ...
  const envPath = join(tmpDir, ".env");
  writeFileSync(envPath, "SZROUTE_LANG=de\n", "utf8");
  ...
  try {
    ...
    assert.equal(exitCode, 0);
    const content = readFileSync(envPath, "utf8");
    assert.ok(content.includes("SZROUTE_LANG=pt-BR"), "Deve ter atualizado para pt-BR");
    const matches = content.match(/SZROUTE_LANG=/g);
    assert.equal(matches?.length, 1, "Deve ter exatamente uma ocorrência de SZROUTE_LANG");
  } finally {
    console.log = origLog;
    resetForTests();
  }
