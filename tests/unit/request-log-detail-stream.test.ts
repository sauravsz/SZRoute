...
test("status discrepancy shows both SZRoute and provider statuses", () => {
  const html = renderToStaticMarkup(
    React.createElement(RequestLoggerDetail, {
      log: {
        status: 504,
  ...
  assert.notEqual(
    html.indexOf("SZRoute returned 504"),
    -1,
    "Should indicate SZRoute returned its own status"
  );
});
