...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "szroute-autostart-linux-"));
...
test("resolveCliPath finds szroute.mjs from argv", async () => {
  ...
  const unitPath = join(tmpDir, ".config", "systemd", "user", "szroute.service");
  const desktopPath = join(tmpDir, ".config", "autostart", "szroute.desktop");
  ...
  if (existsSync(unitPath)) {
    ...
    assert.match(unit, /ExecStart=.*szroute\.mjs.*serve --no-open/m);
