...
function createTempRoot(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "szroute-sync-env-"));
}
...
test("syncEnv creates .env from .env.example and generates install-time secrets", () => {
  ...
  // find the user's real DB at ~/.szroute/ during tests
  ...
  try {
    ...
    assert.match(envContent, /^MACHINE_ID_SALT=szroute-/m);
