...
test("compression status chama szroute_compression_status via mcp", async () => {
  ...
  assert.equal(capturedBody.name, "szroute_compression_status");
});
...
test("compression configure envia configuração via mcp", async () => {
  ...
  globalThis.fetch = origFetch;
  assert.equal(capturedBody.name, "szroute_compression_configure");
  assert.equal(capturedBody.arguments.engine, "caveman");
  assert.ok(capturedBody.arguments.caveman?.aggressiveness === 0.8);
});

test("compression engine set chama szroute_set_compression_engine", async () => {
  let capturedBody: any = null;
  const origFetch = globalThis.fetch;
  globalThis.fetch = ((_url: string, opts: any) => {
    if (opts?.body) capturedBody = JSON.parse(opts.body);
  ...
  globalThis.fetch = origFetch;
  assert.equal(capturedBody.name, "szroute_set_compression_engine");
