...
describe("getAutoUpdateConfig", () => {
  it("defaults to npm or source mode locally", () => {
    const config = autoUpdate.getAutoUpdateConfig({ DATA_DIR: "/tmp/szroute" });
    assert.ok(config.mode === "npm" || config.mode === "source");
    assert.equal(config.repoDir, "/workspace/szroute");
  ...
  it("reads docker-compose settings from env", () => {
    const config = autoUpdate.getAutoUpdateConfig({
      ...
      AUTO_UPDATE_REPO_DIR: "/srv/szroute",
      AUTO_UPDATE_COMPOSE_FILE: "/srv/szroute/docker-compose.yml",
      AUTO_UPDATE_COMPOSE_PROFILE: "base",
      AUTO_UPDATE_SERVICE: "szroute-base",
    ...
    assert.equal(config.repoDir, "/srv/szroute");
    assert.equal(config.composeFile, "/srv/szroute/docker-compose.yml");
    assert.equal(config.composeProfile, "base");
    assert.equal(config.composeService, "szroute-base");
...
describe("buildSourceUpdateScript", () => {
  it("includes git checkout, env sync, and rebuild steps", () => {
    ...
    assert.match(script, /pm2 restart szroute --update-env \|\| true/);
  });
});
