 ...
 *
 * Assertions cover:
 * - exec permission gates child_process behind SZROUTE_PLUGINS_ALLOW_EXEC==="1"
...
describe("pluginWorker sandbox — exec permission gating", () => {
  it("gates child_process behind SZROUTE_PLUGINS_ALLOW_EXEC === '1'", () => {
    assert.ok(
      source.includes('SZROUTE_PLUGINS_ALLOW_EXEC !== "1"') ||
        source.includes("SZROUTE_PLUGINS_ALLOW_EXEC !== '1'"),
      "exec block must check process.env.SZROUTE_PLUGINS_ALLOW_EXEC !== \"1\""
    );
  });

  it("checks the env flag inside the exec permission block", () => {
    // The env guard must appear inside the exec permission block.
    // Use the SECOND occurrence of SZROUTE_PLUGINS_ALLOW_EXEC (the first is in
    ...
    const envIdxInBlock = source.indexOf("SZROUTE_PLUGINS_ALLOW_EXEC", execIdx);
    assert.ok(
      envIdxInBlock !== -1,
      "SZROUTE_PLUGINS_ALLOW_EXEC check must appear inside the exec permission block"
    );
  });
  ...
  it("does NOT wire child_process without the env guard in place", () => {
    ...
    // The SZROUTE_PLUGINS_ALLOW_EXEC check must come before sandbox.child_process.
    const envIdx = source.indexOf("SZROUTE_PLUGINS_ALLOW_EXEC");
...
describe("pluginWorker sandbox — trust-model comment", () => {
  ...
  it("references the SZROUTE_PLUGINS_ALLOW_EXEC opt-in in the comment", () => {
    assert.ok(
      source.includes("SZROUTE_PLUGINS_ALLOW_EXEC"),
      "trust-model comment must reference SZROUTE_PLUGINS_ALLOW_EXEC"
    );
  });
});
