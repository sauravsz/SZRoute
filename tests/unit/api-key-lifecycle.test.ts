...
test.after(() => {
  fs.rmSync(TEST_DATA_DIR, { recursive: true, force: true });
  if (ORIGINAL_SZROUTE_API_KEY === undefined) delete process.env.SZROUTE_API_KEY;
  else process.env.SZROUTE_API_KEY = ORIGINAL_SZROUTE_API_KEY;
...
test("validateApiKey accepts configured environment API keys", async () => {
  process.env.SZROUTE_API_KEY = "sk-env-lifecycle-test";
