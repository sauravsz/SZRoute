...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "szroute-model-sync-route-"));
...
test("model sync route skips success log when fetched models do not change stored models", async () => {
  ...
  globalThis.fetch = async (url) => {
    ...
    assert.equal(
      String(url),
      `http://127.0.0.1:21128/api/providers/${connection.id}/models?refresh=true`
    );
...
test("model sync route falls back to in-process discovery when internal self-fetch throws", async () => {
  ...
  // ::1 (IPv6) resolution issues in containers. PORT defaults to 21128 when env unset.
  const expectedPort = process.env.SZROUTE_PORT || process.env.PORT || "21128";
