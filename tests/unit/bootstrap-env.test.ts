...
function withTempEnv(fn) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "szroute-bootstrap-test-"));
  ...
  try {
    fn({ tempRoot, tempCwd, tempHome, dataDir: path.join(tempHome, ".szroute") });
  } finally {
...
test("bootstrapEnv prefers ~/.szroute/.env over server.env", () => {
  withTempEnv(({ dataDir }) => {
