...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "szroute-combo-context-relay-"));
  ...
  return new Response(
    ...
    }),
    {
      ...
      headers: {
        ...
        "x-szroute-provider-breaker": "open",
      },
    }
  );
}
...
test("handleComboChat context-relay persists a handoff when codex quota reaches the warning threshold", async () => {
  ...
  const result = await handleComboChat({
    ...
    handleSingleModel: async (body) => {
      if (body._szrouteInternalRequest === "context-handoff") {
        summaryCalls += 1;
        return okResponse({
          choices: [
            {
...
test("handleComboChat universal handoff detects model switch before recording current model", async () => {
  ...
  const result = await handleComboChat({
    ...
    handleSingleModel: async (body) => {
      if (body._szrouteInternalRequest === "universal-handoff") {
        summaryCalls += 1;
        return okResponse({
          choices: [
            {
