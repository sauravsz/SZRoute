...
// script, not process.cwd(). When SZRoute is installed globally, the user's
...
test("getCurrentVersion resolves the real version from a foreign cwd (#3295)", async () => {
  ...
  const foreignCwd = mkdtempSync(path.join(tmpdir(), "szroute-cwd-"));
  try {
...
test("createBackup resolves bin/ from a foreign cwd and copies cli/ recursively (#3295)", async () => {
  ...
  const foreignCwd = mkdtempSync(path.join(tmpdir(), "szroute-cwd-"));
  const fakeHome = mkdtempSync(path.join(tmpdir(), "szroute-home-"));
  try {
    process.chdir(foreignCwd); // no bin/ here → cwd-relative binPath would be missing
    process.env.HOME = fakeHome; // redirect ~/.szroute/backups
    mkdirSync(fakeHome, { recursive: true });

    const backupDir = await update.createBackup();

    assert.ok(backupDir, "createBackup must return a path (not null)");
    // szroute.mjs is a real file in bin/ and must be copied
    assert.ok(existsSync(path.join(backupDir, "szroute.mjs")), "szroute.mjs copied");
