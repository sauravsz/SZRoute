...
test("Image URL base: SZROUTE_PUBLIC_BASE_URL wins and strips accidental /v1", async () => {
  await withEnv(
    {
      SZROUTE_PUBLIC_BASE_URL: " http://192.168.107.55:21128/v1/ ",
      NEXT_PUBLIC_BASE_URL: "http://localhost:21128",
    },
    async () => {
      assert.equal(
        __derivePublicBaseUrlForTesting({ host: "localhost:21128" }),
        "http://192.168.107.55:21128"
      );
    }
  );
});
...
test("Image URL base: local NEXT_PUBLIC_BASE_URL does not mask LAN Host header", async () => {
  await withEnv(
    ...
    async () => {
      assert.equal(
        __derivePublicBaseUrlForTesting({ host: "192.168.107.55:21128" }),
        "http://192.168.107.55:21128"
      );
    }
  );
...
test("Image URL base: non-local SZROUTE_BASE_URL remains a compatibility fallback", async () => {
  await withEnv({ SZROUTE_BASE_URL: "https://omni.example.com/v1" }, async () => {
...
test("Executor MODEL_MAP: dot-form SZRoute IDs translate to dash-form ChatGPT slugs", async () => {
  reset();
  const m = installMockFetch();
  try {
...
test("Image gen: file-service:// pointer resolves to download URL and is appended as markdown (non-streaming)", async () => {
  ...
  try {
    ...
    // from the SZRoute cache; clients see a stable /v1/chatgpt-web/image
...
test("Image gen: sediment:// pointer prefers /files/<id>/download over /attachment", async () => {
  ...
  try {
    ...
    // bytes that are cached behind the emitted SZRoute image URL.
    assert.match(
      content,
...
test("Image gen: signed URL bytes are cached and exposed via /v1/chatgpt-web/image URL", async () => {
  ...
  // so we fetch the bytes, cache them locally, and emit an SZRoute image URL.
  ...
  tls.__setTlsFetchOverrideForTesting(async (url, opts = {}) => {
    ...
    if (u.startsWith(downloadUrl)) {
      ...
      // back into bytes before putting the image in SZRoute's cache.
      return {
        status: 200,
        headers: makeHeaders({ "Content-Type": "image/png" }),
    ...
    async () => {
      try {
        ...
        const m = content.match(
          /!\[image\]\((http:\/\/192\.168\.107\.55:21128\/v1\/chatgpt-web\/image\/([a-f0-9]+))\)/
        );
...
test("Image edit: cached SZRoute image URL continues the saved ChatGPT conversation", async () => {
  reset();
  ...
  const imageUrl = `http://192.168.107.55:21128/v1/chatgpt-web/image/${imageId}`;
  const m = installMockFetch();
  try {
    const executor = new ChatGptWebExecutor();
    await executor.execute({
...
test("Image cache: byte cap evicts oldest before count cap kicks in", async () => {
  ...
  const original = process.env.SZROUTE_CGPT_WEB_IMAGE_CACHE_MAX_MB;
  process.env.SZROUTE_CGPT_WEB_IMAGE_CACHE_MAX_MB = "10";
  try {
    ...
    if (original == null) delete process.env.SZROUTE_CGPT_WEB_IMAGE_CACHE_MAX_MB;
    else process.env.SZROUTE_CGPT_WEB_IMAGE_CACHE_MAX_MB = original;
    cacheMod.__resetChatGptImageCacheForTesting();
  }
});
...
test("Image edit handler: no cached match returns 400 (does not silently generate unrelated image)", async () => {
  ...
  try {
    ...
    assert.match(String((result as any).error), /generated through this SZRoute instance/);
