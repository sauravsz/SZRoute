...
test("C2 header contract — server.cjs intercept must inject x-szroute-source and x-szroute-agent", async () => {
  ...
  assert.match(
    src,
    /"x-szroute-source":\s*"agent-bridge"/,
    'server.cjs must inject "x-szroute-source: agent-bridge"'
  );
  assert.match(
    src,
    /"x-szroute-agent":\s*agentId/,
    'server.cjs must inject "x-szroute-agent: <id>" from the host→agent map'
  );
