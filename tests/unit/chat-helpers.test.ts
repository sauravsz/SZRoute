...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "szroute-chat-helpers-"));
...
test("checkPipelineGates blocks providers with an open circuit breaker", async () => {
  ...
  assert.equal(response.headers.get("X-SZRoute-Provider-Breaker"), "open");
});
...
test("withSessionHeader adds headers to mutable and immutable responses", async () => {
  ...
  assert.equal(mutable.headers.get("X-SZRoute-Session-Id"), "sess_mutable");
  assert.equal(immutable.headers.get("X-SZRoute-Session-Id"), "sess_redirect");
