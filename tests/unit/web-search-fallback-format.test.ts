...
const {
  ...
  SZROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME,
...
// szroute_web_search tool must use the FLAT function shape ({ type, name }), not the
...
test("#2390 web_search fallback is FLAT for Responses API target", () => {
  ...
  assert.equal(injected.name, SZROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME);
  assert.equal(
    injected.function,
    undefined,
...
test("#2390 web_search fallback stays NESTED for Chat Completions target", () => {
  ...
  assert.equal(fn?.name, SZROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME);
  assert.equal(
    injected.name,
    undefined,
...
test("#2390 tool_choice matches the injected tool shape per target format", () => {
  ...
  assert.equal(rChoice.name, SZROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME);
  ...
  assert.equal(cFn?.name, SZROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME);
});
...
test("OpenAI -> Claude (non-passthrough): built-in web_search IS still converted", () => {
  ...
  assert.equal(fallback.enabled, true);
  assert.equal(fallback.toolName, SZROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME);
  ...
  assert.ok(toolNames.includes(SZROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME));
});
