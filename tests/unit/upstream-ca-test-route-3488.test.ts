...
const DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "szroute-ca-datadir-"));
...
// A throwaway self-signed cert (CN=SZRoute Test CA), valid to 2036.
...
const dir = fs.mkdtempSync(path.join(os.tmpdir(), "szroute-ca-test-"));
...
test("#3488 valid PEM cert → 200 ok with subject", async () => {
  ...
  assert.match(json.subject, /SZRoute Test CA/);
});
