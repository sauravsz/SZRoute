...
test("electron smoke env allowlists runtime variables and drops secrets", () => {
  ...
  assert.equal(env.DATA_DIR, "/tmp/szroute-electron-smoke-test");
  assert.equal(env.DISPLAY, ":99");
  assert.equal(env.PATH, "/usr/bin");
  assert.equal(env.HOME, "/tmp/szroute-electron-smoke-test/home");
  assert.equal(env.XDG_CONFIG_HOME, "/tmp/szroute-electron-smoke-test/config");
