...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "szroute-context-handoff-"));
...
test("maybeGenerateHandoff persists a structured handoff once the threshold is reached", async () => {
  ...
  assert.equal(calls[0].body._szrouteSkipContextRelay, true);
  assert.equal(calls[0].body._szrouteInternalRequest, "context-handoff");
});
