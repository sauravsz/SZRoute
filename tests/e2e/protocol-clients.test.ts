...
const BASE_URL = process.env.SZROUTE_BASE_URL || "http://localhost:21128";
  ...
  it(
    ...
    async () => {
      const transport = new StdioClientTransport({
        ...
        env: {
          ...process.env,
          SZROUTE_BASE_URL: BASE_URL,
          SZROUTE_API_KEY: API_KEY,
      ...
      try {
        ...
        expect(toolNames).toContain("szroute_get_health");
        expect(toolNames).toContain("szroute_list_combos");
      ...
      const auditRes = await apiFetch("/api/mcp/audit?limit=50&tool=szroute_get_health");
      ...
      if (auditRes.status === 200) {
        ...
        expect(entries.some((entry: any) => entry.toolName === "szroute_get_health")).toBe(true);
      }
    },
    TEST_TIMEOUT_MS * 2
  );
