...
description: "Interact with SZRoute via JSON-RPC 2.0 agent-to-agent protocol. 6 built-in A2A skills: smart-routing, quota-management, provider-discovery, cost-analysis, health-report, list-capabilities."
---
...
<!-- skill:custom-start -->
<!-- Migrated from skills/szroute-a2a/SKILL.md (preserved curated content) -->

# SZRoute — A2A Protocol
...
SZRoute publishes an Agent Card at `/.well-known/agent.json` and accepts
JSON-RPC 2.0 calls at `/a2a`.

## Discovery

```bash
curl $SZROUTE_URL/.well-known/agent.json
```
...
- `401` → missing/invalid `SZROUTE_KEY`

## Reference

Full docs: https://github.com/sauravsz/SZRoute/blob/main/docs/frameworks/A2A-SERVER.md
<!-- skill:custom-end -->
