...
If you discover a security vulnerability in SZRoute, please report it responsibly:

1. **DO NOT** open a public GitHub issue
2. Use [GitHub Security Advisories](https://github.com/sauravsz/SZRoute/security/advisories/new)
...
SZRoute implements a multi-layered security model:
...
SZRoute ships a hot-reloadable **guardrails registry** (`src/lib/guardrails/`) with 3 built-in guardrails ordered by priority:
...
Custom guardrails register via `registerGuardrail(new MyGuardrail())`. The model is fail-open (exceptions never block traffic). Per-request opt-out via `x-szroute-disabled-guardrails` header. → See [`docs/security/GUARDRAILS.md`](docs/security/GUARDRAILS.md).
...
The published `szroute` npm artifact bundles the Next.js `output: "standalone"`
...
`SZROUTE_BUILD_PROFILE=minimal npm run build`. That replaces the four
