...
export async function runAvailableCommand(opts = {}) {
  ...
  if (opts.json) {
    ...
    printHeading("SZRoute Available Providers");
...
export async function runTestAllCommand(opts = {}) {
  ...
  try {
    ...
    if (opts.json) {
      ...
      printHeading("SZRoute Provider Tests");
...
export async function runValidateCommand(opts = {}) {
  ...
  try {
    ...
    if (opts.json) {
      ...
      printHeading("SZRoute Provider Validation");
      if (results.length === 0) {
        console.log("No providers configured.");
      }
      for (const result of results) {
...
export async function runProvidersRotateCommand(selector, opts = {}) {
  ...
  if (serverUp) {
    try {
      ...
      const { db: db2 } = await openSZRouteDb();
      try {
  ...
  // --- Post-rotation test ---
  if (!opts.skipTest) {
    const { db: db3 } = await openSZRouteDb();
    try {
...
export function registerProviders(program) {
  ...
  providers
    .command("available")
    .description("Show available providers in the SZRoute catalog")
