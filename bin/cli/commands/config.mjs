...
async function runConfigGetCommand(toolId, opts = {}) {
  if (!toolId) {
    printError("Tool ID required. Usage: szroute config get <tool>");
    return 1;
  }
...
async function runConfigSetCommand(toolId, opts = {}) {
  if (!toolId) {
    printError("Tool ID required. Usage: szroute config set <tool> [options]");
    return 1;
  }

  const baseUrl = opts.baseUrl || "http://localhost:21128/v1";
  const apiKey = opts.apiKey;
  const model = opts.model;

  if (!apiKey) {
    printError("API key required. Use --api-key or set SZROUTE_API_KEY.");
    return 1;
  }
...
async function runConfigValidateCommand(toolId, opts = {}) {
  if (!toolId) {
    printError("Tool ID required. Usage: szroute config validate <tool>");
    return 1;
  }
...
export async function runConfigLangSetCommand(code, opts = {}) {
  ...
  upsertEnvLine(envPath, "SZROUTE_LANG", code);
  setLocale(code);
  ...
  config
    ...
    .action(async (tool, opts, cmd) => {
      ...
      const exitCode = await runConfigSetCommand(tool, {
        ...opts,
        apiKey: opts.apiKey || globalOpts.apiKey || process.env.SZROUTE_API_KEY,
        baseUrl: opts.baseUrl || globalOpts.baseUrl || process.env.SZROUTE_BASE_URL,
