...
export function registerPlugin(program) {
  const plugin = program
    .command("plugin")
    .description(t("plugin.description") || "Manage CLI plugins (szroute-cmd-*)");
  ...
  plugin
    ...
    .action(async (opts, cmd) => {
      ...
      if (plugins.length === 0) {
        ...
        process.stdout.write(`Install: szroute plugin install <name>\n`);
      }
    });
  ...
  plugin
    ...
    .action(async (name, opts) => {
      ...
      const pkgName = isLocal ? name : `szroute-cmd-${name}`;

      if (!opts.yes) {
        process.stderr.write(
          `⚠ WARNING: Plugins run with the same privileges as szroute CLI.\n` +
  ...
  plugin
    ...
    .action(async (name, opts) => {
      const pkgName = name.startsWith("szroute-cmd-") ? name : `szroute-cmd-${name}`;
      if (!opts.yes) {
  ...
  plugin
    ...
    .action(async (name, opts, cmd) => {
      const plugins = await discoverPlugins();
      const p = plugins.find((x) => x.name === name || x.name === `szroute-cmd-${name}`);
      if (!p) {
  ...
  plugin
    ...
    .action(async (query) => {
      const q = query ? `szroute-cmd-${query}` : "szroute-cmd";
      ...
      try {
        ...
        if (rows.length === 0) {
          process.stdout.write(`No plugins found for '${query || "szroute-cmd"}'.\n`);
        } else {
          rows.forEach((r) =>
  ...
  plugin
    ...
    .action(async (name) => {
      ...
      const dir = join(process.cwd(), `szroute-cmd-${safeName}`);
      if (existsSync(dir)) {
      ...
      writeFileSync(
        ...
        JSON.stringify(
          {
            name: `szroute-cmd-${safeName}`,
            version: "0.1.0",
            type: "module",
            main: "index.mjs",
            description: `SZRoute CLI plugin: ${safeName}`,
      ...
      writeFileSync(
        join(dir, "README.md"),
        `# szroute-cmd-${safeName}\n\nAn SZRoute CLI plugin.\n\n## Install\n\n\`\`\`bash\nszroute plugin install ${safeName}\n\`\`\`\n`
      );
      process.stdout.write(`✓ Scaffolded: ${dir}\n`);
      process.stdout.write(`  Run: cd ${dir} && szroute plugin install .\n`);
    });
}
