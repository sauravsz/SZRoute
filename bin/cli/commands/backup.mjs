...
export function registerBackup(program) {
  ...
  // Legacy: `szroute backup` without subcommand still creates a backup
...
async function pruneBackups(backupDir, retention) {
  ...
  try {
    const dirs = readdirSync(backupDir)
      .filter((f) => f.startsWith("szroute-backup-"))
      .sort()
      .reverse();
...
export async function runBackupCommand(opts = {}) {
  ...
  const backupName = safeName ? `szroute-backup-${safeName}` : `szroute-backup-${timestamp}`;
...
async function _uploadBackupToCloud(backupPath, info) {
  ...
  try {
    const boundary = `szroute-backup-${Date.now().toString(36)}-${randomBytes(8).toString("hex")}`;
    const headers = new Headers({
      accept: "application/json",
      "content-type": `multipart/form-data; boundary=${boundary}`,
    });
    const apiKey = process.env.SZROUTE_API_KEY;
    if (apiKey) headers.set("authorization", `Bearer ${apiKey}`);
    const cliToken = process.env.SZROUTE_CLI_TOKEN ?? (await getCliToken());
...
export async function runRestoreCommand(backupId, opts = {}) {
  ...
  if (opts.list || !backupId) {
    ...
    if (!backupId) console.log("\nUsage: szroute restore <backup-id>");
    return 0;
  }

  const safeBackupId = String(backupId).replace(/[/\\]/g, "_");
  const backupPath = join(backupDir, `szroute-backup-${safeBackupId}`);
