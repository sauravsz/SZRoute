...
const RUNTIME_DIR = join(homedir(), ".szroute", "runtime");
...
export async function loadSqliteRuntime() {
  ...
  try {
    ...
    console.warn(`[szroute] runtime install failed: ${err.message}`);
  }
  ...
  if (existsSync(buildDir)) {
    ...
    if (nodeFile) {
      ...
      if (!magic || (magic !== expected && magic !== "macho-le" && magic !== "macho-fat")) {
        console.warn(
          `[szroute] runtime sqlite binary magic mismatch (${magic} ≠ ${expected}) — skipping`
        );
        return null;
      }
    }
...
function ensureRuntimeDir() {
  ...
  if (!existsSync(pkg)) {
    writeFileSync(
      pkg,
      JSON.stringify({ name: "szroute-runtime", private: true, type: "commonjs" }),
      "utf-8"
    );
  }
}
