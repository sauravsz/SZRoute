 ...
 *
 * Purely informational invocations must NOT create `~/.szroute/.env` or write
 * a key — they never touch encrypted storage. Generating a 32-byte key and a
 * `.env` file just to print `szroute --version` (or `--help`) is a surprising
 ...
 *
 * Returns TRUE for everything else, INCLUDING a bare `szroute` (no args) — the
...
export function shouldProvisionStorageKey(argv) {
  ...
  // Bare `szroute` runs the default `serve` command → must provision.
