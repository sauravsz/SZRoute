# bin/cli — SZRoute CLI internals

This directory contains the CLI runtime, helpers, and commands for the `szroute` binary.
...
├── sqlite.mjs              ← openSZRouteDb() — DB bootstrap
...
All HTTP calls to the SZRoute server must go through this wrapper.
...
- `baseUrl` — override base URL (default: `SZROUTE_BASE_URL` env or `localhost:21128`)
- `apiKey` — override API key (default: `SZROUTE_API_KEY`)
...
Locale detection order: `SZROUTE_LANG` → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
...
2. `SZROUTE_LANG` environment variable
...
```bash
szroute config lang set pt-BR       # saves to ~/.szroute/.env
szroute config lang list            # show all 42 available locales
szroute config lang get             # show currently active locale
```

**One-time override:**

```bash
szroute --lang de providers list    # run in German, not persisted
SZROUTE_LANG=ja szroute status    # same effect via env
```
