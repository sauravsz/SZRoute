# SZRoute CLI — Internal Conventions
...
```
szroute keys add openai sk-xxx
szroute combo switch fastest
szroute memory search "react hooks"
```

**Not allowed**:

```
szroute --add-key openai sk-xxx     # ❌ flag-as-verb
szroute add-key openai sk-xxx       # ❌ hyphen at the top level
```
...
- Reads base URL from `SZROUTE_BASE_URL` env or `~/.szroute/config.json`
  (active profile).
- Injects `Authorization: Bearer ${SZROUTE_API_KEY}` when available.
- Injects `x-szroute-cli-token` when applicable (see task 8.12).
...
- Detection order: `--lang` flag → `SZROUTE_LANG` env → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
- Locale persisted via `config lang set <code>` — saves `SZROUTE_LANG` to `~/.szroute/.env`.
