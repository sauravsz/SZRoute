...
Use \`createCombo\` tool or \`runSZRouteCli\` to create them.`;
  }
...
SZRoute supports 212+ providers across categories:
...
I can use CodeGraph to explore the SZRoute codebase. Just ask me:
...
function getHelpResponse(): string {
  return `## SZRoute Copilot — Comandos disponibles
...
### CLI
- "CLI health" → ejecuta \`szroute health\`
- "CLI list-combos" → ejecuta \`szroute list-combos\`
- "CLI set-budget 10" → ejecuta \`szroute set-budget 10\`

### Conocimiento
- "Cómo funciona SZRoute?" → explica la arquitectura
...
export async function processCopilotChat(request: CopilotRequest): Promise<CopilotResponse> {
  ...
  if (!intent) {
    ...
    return {
      message: `I understand you want help with SZRoute.\n\n${getHelpResponse()}`,
    };
  }
...
> Crea un combo llamado "fallback-claude" con estrategia priority y targets: [{"provider":"claude-code","model":"claude-sonnet-4"},{"provider":"openai","model":"gpt-4o"}]`,
  ...
  if (intent.tool === "runSZRouteCli") {
    const tool = getCopilotTool("runSZRouteCli");
    if (!tool) return { message: "Error: CLI executor not found." };

    const result = await tool.handler(intent.args);
    return {
      message: result,
      toolCalls: [{ name: "runSZRouteCli", args: intent.args, result }],
    };
  }
