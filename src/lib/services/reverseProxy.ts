 ...
 * can be iframed inside the SZRoute dashboard without CORS issues.
 *
 * Security:
 ...
 *     to prevent credential leakage between SZRoute and the embedded service.
 ...
 * Prevents SZRoute session cookies and Authorization from leaking upstream.
 */
 ...
 *   SZRoute origin, which would conflict with session management.
 * - content-security-policy / content-security-policy-report-only: the
 *   embedded service's CSP is irrelevant inside the SZRoute iframe.
 * - x-frame-options: would block the iframe entirely if set to DENY/SAMEORIGIN
 *   by the embedded service (SZRoute controls framing via its own CSP).
