...
import { resolveModelAlias } from "@szroute/open-sse/services/modelDeprecation.ts";
import {
...
function isRedisAuthCacheEnabled(): boolean {
  return (
    process.env.SZROUTE_DISABLE_REDIS_AUTH_CACHE !== "1" &&
...
export async function getApiKeyMetadata(
  ...
  if (isConfiguredEnvApiKey(key)) {
    ...
    // The deployment-time env key (`SZROUTE_API_KEY` / `ROUTER_API_KEY`)
