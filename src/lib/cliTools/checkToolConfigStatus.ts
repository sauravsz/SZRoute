...
/**
 * Check if a tool has SZRoute configured by reading its config file directly.
  ...
  try {
    ...
    if (toolId === "hermes") {
      ...
      const hasSZRoute =
        ...
        lower.includes(`localhost:${apiPort}`) ||
        lower.includes(`127.0.0.1:${apiPort}`);
      return hasSZRoute ? "configured" : "not_configured";
    }

    const config = JSON.parse(content) as Record<string, unknown>;

    // Each tool stores SZRoute config differently
    switch (toolId) {
      case "claude":
      ...
      case "cline":
      case "kilo": {
        // Generic check: look for SZRoute-specific markers in the config
        const configStr = JSON.stringify(config).toLowerCase();
        if (
          configStr.includes("szroute") ||
          configStr.includes("sk_szroute") ||
