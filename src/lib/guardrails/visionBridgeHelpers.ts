 ...
 *      SZRoute itself, OpenRouter, a Gemini-OpenAI-compat URL).
 ...
 *      SZRoute's own `/v1` so the vision model can be any provider
 *      registered in SZRoute (`google/gemini-2.0-flash`,
 *      `openrouter/...`, etc.) instead of being limited to OpenAI/Anthropic.
 *   2. `OPENAI_API_URL` env var (legacy)
 *   3. SZRoute self-loop (`http://localhost:21128/v1`) — auto-detected when
 *      the model uses a known SZRoute-internal provider (e.g. `kr/`, `if/`,
 ...
 *                that require SZRoute self-loop routing.
 */
export function resolveVisionBridgeBaseUrl(model?: string): string {
  ...
  // anthropic/), it can only be resolved through SZRoute's own router,
  // not through a direct OpenAI/Anthropic endpoint. Use the operator-configured
  // port via SZROUTE_PORT / PORT env vars, falling back to the default 21128.
...
async function callVisionModelSingle(
  ...
  try {
    ...
    if (isAnthropic) {
      ...
      // SZRoute itself or any other OpenAI-compatible endpoint instead of
      // hardcoded api.openai.com.
      const baseUrl = resolveVisionBridgeBaseUrl(config.model);

      // When routing through the SZRoute self-loop (non-standard provider),
      // keep the full provider-prefixed model ID so SZRoute can resolve the
      ...
      // When routing through SZRoute's own API, omit the vision-bridge
      // guardrail on the sub-request to prevent infinite recursion.
      // Use sk_szroute as fallback for self-loop if no API key is resolved.
      const selfLoopApiKey = resolvedApiKey || "sk_szroute";
      const headers: Record<string, string> = {
        "Content-Type": "application/json",
        Authorization: `Bearer ${selfLoopApiKey}`,
      };
      if (useFullModelId) {
        headers["x-szroute-disabled-guardrails"] = "vision-bridge";
      }
