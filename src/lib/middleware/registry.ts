...
declare global {
  var __szroutePreRequestRegistry:
    | {
        initialized: boolean;
...
function getRegistryState() {
  if (!globalThis.__szroutePreRequestRegistry) {
    globalThis.__szroutePreRequestRegistry = {
      initialized: false,
      hooks: new Map(),
      middlewares: new Map(),
      logs: [],
  ...
  return globalThis.__szroutePreRequestRegistry;
}
