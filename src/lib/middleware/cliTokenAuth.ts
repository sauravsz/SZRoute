...
const HEADER_NAME = "x-szroute-cli-token";
...
/**
 * Validates the CLI machine-id token sent by the local szroute CLI.
 * Only accepted from loopback IPs. Disabled via SZROUTE_DISABLE_CLI_TOKEN=true.
 */
export async function isCliTokenAuthValid(request: Request): Promise<boolean> {
  if (process.env.SZROUTE_DISABLE_CLI_TOKEN === "true") return false;
