...
function getLHCState() {
  if (!globalThis.__szrouteLocalHC) {
    globalThis.__szrouteLocalHC = {
      initialized: false,
      sweepTimer: null,
      healthCache: new Map(),
  ...
  return globalThis.__szrouteLocalHC;
}
...
function isLocalHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("SZROUTE_DISABLE_LOCAL_HEALTHCHECK") ||
    isBuildProcess() ||
