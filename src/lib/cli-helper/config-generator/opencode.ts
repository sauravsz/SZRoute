 ...
 * source is the user's OWN SZRoute instance, so loopback/private hosts are the
...
/**
 * Fetch the live `/v1/models` catalog from SZRoute. The catalog is the
 ...
 */
export async function fetchSZRouteCatalog(
  baseUrl: string,
  apiKey: string,
  timeoutMs = 5_000
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `SZRoute /v1/models returned ${response.status} ${response.statusText}`
      );
    }
...
/**
 * Generate a full `opencode.json` document for SZRoute. The catalog is the
...
export async function generateOpencodeConfig(
  ...
  const providerId = options.providerId?.trim() || "szroute";
  ...
  if (fetchCatalog) {
    const result = await fetchSZRouteCatalog(baseURL, options.apiKey, timeoutMs);
  ...
  const provider: Record<string, unknown> = {
    name: existingProvider?.name ?? "SZRoute",
