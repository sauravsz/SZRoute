 ...
 * Priority: system daemon socket → SZRoute custom socket.
...
async function getActiveSocketPath(): Promise<string> {
  ...
  // Fallback to SZRoute custom socket
...
async function getDefaultHostname() {
  try {
    ...
    const normalized = `szroute-${machineId.slice(0, 8)}`.replace(/[^a-zA-Z0-9-]/g, "-");
