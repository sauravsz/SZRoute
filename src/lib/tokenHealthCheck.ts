...
import {
  ...
  refreshCopilotToken,
} from "@szroute/open-sse/services/tokenRefresh.ts";
...
function isHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("SZROUTE_DISABLE_TOKEN_HEALTHCHECK") ||
    isBuildProcess() ||
 ...
 * SZROUTE_DISABLE_TOKEN_HEALTHCHECK switch: it lets an operator keep the
...
function getHealthCheckSkipProviders(): Set<string> {
  const raw = process.env.SZROUTE_HEALTHCHECK_SKIP_PROVIDERS || "";
  return new Set(
    raw
      .split(",")
...
async function shouldHideLogs(): Promise<boolean> {
  if (
    isEnvFlagEnabled("SZROUTE_HIDE_HEALTHCHECK_LOGS") ||
    isBuildProcess() ||
    isAutomatedTestProcess()
  ) {
    return true;
...
declare global {
  var __szrouteTokenHC:
    | { initialized: boolean; interval: ReturnType<typeof setInterval> | null }
    | undefined;
}

function getHCState() {
  if (!globalThis.__szrouteTokenHC) {
    globalThis.__szrouteTokenHC = { initialized: false, interval: null };
  }
  return globalThis.__szrouteTokenHC;
}
...
export async function checkConnection(conn) {
  ...
  // Codex). 9router did not have this background sweep; it was introduced in SZRoute
