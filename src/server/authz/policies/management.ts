...
function requestPeerAddress(ctx: PolicyContext): string | null {
  ...
  const stamped = resolveStampedPeer(
    ...
    process.env.SZROUTE_PEER_STAMP_TOKEN
  );
...
const WS_BRIDGE_SECRET_HEADER = "x-szroute-ws-bridge-secret";
...
// server-ws.mjs (SZROUTE_WS_BRIDGE_SECRET). Without this carve-out the MANAGEMENT
...
function isValidWsBridgeRequest(ctx: PolicyContext): boolean {
  ...
  const expected = process.env.SZROUTE_WS_BRIDGE_SECRET || "";
