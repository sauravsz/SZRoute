...
// POST — write SZRoute settings to jcode config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge SZRoute settings (jcode uses OpenAI-compatible config)
...
// DELETE — remove SZRoute settings from jcode config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "jcode SZRoute settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
