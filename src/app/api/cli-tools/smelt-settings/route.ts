...
/**
 * Check if the config file contains SZRoute settings.
 */
const hasSZRouteConfig = (settings: Record<string, unknown> | null): boolean => {
  if (!settings) return false;
  return (
    typeof settings.baseUrl === "string" &&
    settings.baseUrl.length > 0 &&
    settings._managedBy === "szroute"
  );
};
...
// POST — write SZRoute settings to Smelt config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge SZRoute settings (smelt uses OpenAI-compatible config)
...
// DELETE — remove SZRoute settings from Smelt config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Smelt SZRoute settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
