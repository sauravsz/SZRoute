...
// POST — write SZRoute settings to Pi config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge SZRoute settings (pi uses OpenAI-compatible config)
...
// DELETE — remove SZRoute settings from Pi config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Pi SZRoute settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
