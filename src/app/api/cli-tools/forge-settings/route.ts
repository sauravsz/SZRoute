...
/**
 * Render the SZRoute provider block in Forge TOML format.
...
function renderForgeConfig(baseUrl: string, apiKey: string, model: string): string {
  ...
  return [
    "# Forge config — managed by SZRoute (plan 14)",
    "",
    "[openai]",
    `api_key = "${apiKey}"`,
 ...
 * Looks for the managed-by-SZRoute marker comment.
 */
const hasSZRouteConfig = (content: string | null): boolean => {
  if (!content) return false;
  return content.includes("managed by SZRoute");
};
...
// POST — write SZRoute settings to Forge config.toml
...
// DELETE — remove Forge SZRoute config
