...
// Check if settings has SZRoute config
const hasSZRouteConfig = (settings: any) => {
  ...
  return openai.some((p: any) => {
    if (p.name?.includes("SZRoute") || p.id === "szroute") return true;
...
// POST - Write SZRoute config to settings.json + .env
export async function POST(request: Request) {
  ...
  try {
    ...
    const envLines = envContent.split("\n").filter((line) => {
      // Remove old SZRoute-related keys we're about to write
      return (
    ...
    // openai provider — primary, supports all models via SZRoute
    const openaiEntry = {
      id: resolvedModel,
      name: `${resolvedModel} (SZRoute)`,
    ...
    const openaiIdx = openaiProviders.findIndex(
      (p: any) => p && (p.baseUrl === normalizedBaseUrl || p.id === "szroute")
    );
    if (openaiIdx >= 0) {
    ...
    // anthropic provider — for Claude models via SZRoute
    const anthropicEntry = {
      id: "claude-sonnet-4-6",
      name: "Claude Sonnet 4.6 (SZRoute)",
    ...
    // gemini provider — for Gemini models via SZRoute
    const geminiEntry = {
      id: "gemini-3-flash",
      name: "Gemini 3 Flash (SZRoute)",
...
// DELETE - Remove SZRoute config from settings.json and .env
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove SZRoute entries from each provider type
    ...
    for (const type of providerTypes) {
      if (Array.isArray(existingConfig.modelProviders?.[type])) {
        existingConfig.modelProviders[type] = existingConfig.modelProviders[type].filter(
          (p: any) => !p.name?.includes("SZRoute") && p.id !== "szroute"
        );
    ...
    return NextResponse.json({
      success: true,
      message: "SZRoute settings removed from Qwen Code",
    });
  } catch (error) {
