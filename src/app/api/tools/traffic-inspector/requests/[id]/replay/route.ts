 ...
 *
 * Re-issues the captured request through the local SZRoute instance and
...
import { buildErrorBody, sanitizeErrorMessage } from "@szroute/open-sse/utils/error.ts";
...
const SZROUTE_BASE = process.env.SZROUTE_BASE_URL ?? "http://127.0.0.1:21128";
...
export async function POST(_request: Request, { params }: Params): Promise<Response> {
  ...
  const url = `${SZROUTE_BASE}${entry.path}`;

  const replayHeaders: Record<string, string> = {
    "content-type": "application/json",
    "x-szroute-source": "inspector-replay",
  };
