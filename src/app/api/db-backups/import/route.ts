...
// Required tables that must exist in a valid SZRoute database
...
export async function POST(request: Request) {
  ...
  try {
    ...
    tmpPath = path.join(os.tmpdir(), `szroute-import-${Date.now()}.sqlite`);
    ...
    try {
      ...
      if (missingTables.length > 0) {
        return NextResponse.json(
          {
            error: `Invalid SZRoute database. Missing tables: ${missingTables.join(", ")}`,
          },
          { status: 400 }
        );
      }
