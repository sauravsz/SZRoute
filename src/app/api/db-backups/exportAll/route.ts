...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const tempDir = path.join(os.tmpdir(), `szroute-export-${timestamp}`);
    const zipPath = path.join(os.tmpdir(), `szroute-full-backup-${timestamp}.zip`);
    ...
    try {
      ...
      return new NextResponse(archiveBuffer, {
        ...
        headers: {
          ...
          "Content-Disposition": `attachment; filename="szroute-full-backup-${timestamp}.tar.gz"`,
