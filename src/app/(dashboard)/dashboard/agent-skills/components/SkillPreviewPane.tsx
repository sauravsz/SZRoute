...
export function SkillPreviewPane({
  ...
  const handleCopyRawUrl = useCallback(async () => {
    if (!skillId) return;
    const rawUrl = `https://raw.githubusercontent.com/sauravsz/SZRoute/refs/heads/main/skills/${skillId}/SKILL.md`;
    try {
  ...
  const githubUrl = skillId
    ? `https://github.com/sauravsz/SZRoute/blob/main/skills/${skillId}/SKILL.md`
    : null;
