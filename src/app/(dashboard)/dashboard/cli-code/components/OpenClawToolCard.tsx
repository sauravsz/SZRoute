...
export default function OpenClawToolCard({
  ...
  const getConfigStatus = () => {
    ...
    const currentProvider = openclawStatus.settings?.models?.providers?.["szroute"];
  ...
  useEffect(() => {
    if (openclawStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = openclawStatus.settings?.models?.providers?.["szroute"];
      if (provider) {
        const primaryModel = openclawStatus.settings?.agents?.defaults?.model?.primary;
        if (primaryModel) {
          const modelId = primaryModel.replace("szroute/", "");
  ...
  const getManualConfigs = () => {
    ...
    const keyToDisplay =
      selectedKeyObj?.key || (!cloudEnabled ? "sk_szroute" : "<API_KEY_FROM_DASHBOARD>");

    const settingsContent = {
      agents: {
        defaults: {
          model: {
            primary: `szroute/${selectedModel || "provider/model-id"}`,
          },
        },
      },
      models: {
          ...
          {!checkingOpenclaw && cliReady && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current Base URL */}
                {openclawStatus?.settings?.models?.providers?.["szroute"]?.baseUrl && (
                  <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={!openclawStatus?.hasSZRoute}
                  loading={restoring}
                >
