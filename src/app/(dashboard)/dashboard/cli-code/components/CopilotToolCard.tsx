 ...
 *
 * Feature request: https://github.com/sauravsz/SZRoute/issues/142
 */
export default function CopilotToolCard({
  ...
  const [selectedModels, setSelectedModels] = useState<Set<string>>(() => {
    ...
    try {
      const saved = localStorage.getItem("szroute-copilot-selected-models");
  ...
  const [selectedApiKeyId, setSelectedApiKeyId] = useState(() => {
    if (typeof window !== "undefined") {
      const savedKey = localStorage.getItem("szroute-cli-key-copilot");
  ...
  useEffect(() => {
    if (selectedModels.size > 0) {
      localStorage.setItem(
        "szroute-copilot-selected-models",
  ...
  const generateConfig = () => {
    ...
    const config = {
      name: "SZRoute",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.szroute}`,
      models,
    };

    const responsesConfig = {
      name: "SZRoute-responses",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.szroute}`,
  ...
  const handleApiKeyChange = (value: string) => {
    ...
    if (value) localStorage.setItem("szroute-cli-key-copilot", value);
  };
