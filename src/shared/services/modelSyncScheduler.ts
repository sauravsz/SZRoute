...
const globalState = globalThis as typeof globalThis & {
  __szrouteModelSyncInternalAuthToken?: string;
};
...
function getInternalAuthToken(): string {
  if (!internalAuthToken) {
    internalAuthToken = globalState.__szrouteModelSyncInternalAuthToken || randomUUID();
    globalState.__szrouteModelSyncInternalAuthToken = internalAuthToken;
  }
...
export function isModelSyncInternalRequest(request: { headers: Headers }): boolean {
  if (!internalAuthToken && globalState.__szrouteModelSyncInternalAuthToken) {
    internalAuthToken = globalState.__szrouteModelSyncInternalAuthToken;
  }
 ...
 * @param apiBaseUrl — internal base URL to call SZRoute's own API
