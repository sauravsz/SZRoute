/**
 * Shared Node.js runtime support policy for SZRoute.
 *
...
export function getNodeRuntimeWarning(version: string = process.versions.node): string | null {
  ...
  if (support.reason === "unreleased-major") {
    return `Node.js ${support.nodeVersion} is outside the supported LTS lines. SZRoute currently supports Node.js 20.x, 22.x, 24.x, 25.x, and 26.x.`;
  }

  return `Node.js ${support.nodeVersion} is outside SZRoute's approved secure runtime policy.`;
}
