...
import { getClaudeCodeDefaultModels } from "@szroute/open-sse/config/providerRegistry";
...
export const CLI_TOOLS: Record<string, CliCatalogEntry> = {
  claude: {
    ...
    icon: "terminal",
    color: "#D97757",
    description: "Anthropic Claude Code CLI — ANTHROPIC_BASE_URL points to SZRoute",
  ...
  codex: {
    ...
    color: "#10A37F",
    description: "OpenAI Codex CLI — OpenAI-compatible base URL targets SZRoute",
  ...
  opencode: {
    ...
    notes: [
      ...
      {
        type: "warning",
        text: 'Thinking variant example: opencode run "implement this feature" --model szroute/claude-sonnet-4-5-thinking --variant high',
      },
    ],
    guideSteps: [
  ...
  hermes: {
    ...
    guideSteps: [
      ...
      { step: 4, title: "Select Model", type: "modelSelector" },
      {
        ...
        desc: "Use the JSON block below as the OpenAI-compatible provider definition for SZRoute.",
      },
    ],
    codeBlock: {
      language: "json",
  ...
  qwen: {
    ...
    color: "#10B981",
    description:
      "Alibaba Qwen Code CLI — supports OpenAI, Anthropic & Gemini providers via SZRoute",
    ...
    notes: [
      {
        type: "info",
        text: "Qwen Code supports multiple provider types (openai, anthropic, gemini) via modelProviders in settings.json. SZRoute works as an OpenAI-compatible endpoint.",
      },
      {
        type: "info",
        text: "Any model available in SZRoute can be used — not just Qwen models. Select from Qwen, Claude, Gemini, GPT, and more.",
      },
      {
        type: "warning",
      ...
      {
        type: "error",
        text: "Qwen OAuth free tier was discontinued on 2026-04-15. Use SZRoute with bailian-coding-plan/alibaba/alibaba-cn/openrouter/anthropic/gemini providers instead.",
      },
    ],
    modelAliases: [
      "coder-model",
    ...
    codeBlock: {
      language: "json",
      code: `# ~/.qwen/settings.json — SZRoute via security.auth
{
  "security": {
    "auth": {
      "selectedType": "openai",
