...
export default function OAuthModal({
  ...
  const startOAuthFlow = useCallback(async () => {
    ...
    try {
      ...
      // show an auth code instead of redirecting back to SZRoute.
      ...
      // - Windsurf/Devin CLI (remote fallback): use localhost with SZRoute port + /auth/callback
      ...
      //   NEXT_PUBLIC_BASE_URL or SZROUTE_PUBLIC_BASE_URL are configured.
      ...
      if (provider === "codex" || provider === "openai") {
        ...
        // Remote fallback: use SZRoute's port with the /auth/callback path Windsurf expects.
        // On true localhost this code is never reached (callback server handles the flow above).
        const port = window.location.port || "21128";
        ...
        // Behind reverse proxy: use actual origin (e.g., https://szroute.example.com/callback)
        // Supports PUBLIC_URL env var override, or falls back to window.location.origin.
        const publicUrl = process.env.NEXT_PUBLIC_BASE_URL;
        const origin =
          publicUrl && publicUrl !== "http://localhost:21128"
            ? publicUrl.replace(/\/$/, "")
                    ...
                    <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-200">
                      ...
                      <strong>
                        {t.rich("googleOAuthWarning", {
                          ...
                          a: (c) => (
                            <a
                              href="https://github.com/sauravsz/SZRoute#oauth-on-a-remote-server"
                              target="_blank"
                              rel="noreferrer"
                              className="underline"
                            >
