 ...
 *   - forwarding to the SZRoute router (Next.js API)
 *   - SSE piping
 ...
 * Routes through `@szroute/open-sse/utils/error.sanitizeErrorMessage` (Hard Rule #12)
...
async function safeErrorMessage(err: unknown): Promise<string> {
  try {
    const mod = (await import("@szroute/open-sse/utils/error")) as {
...
export abstract class MitmHandlerBase {
   ...
   *   3. Call `this.fetchRouter(...)` for the SZRoute router round-trip.
  ...
  /**
   * Forward the prepared body to the SZRoute router (Next.js API).
   * Adds AgentBridge correlation headers (`x-szroute-source`, `x-szroute-agent`)
  ...
  protected async fetchRouter(
    ...
    const base = process.env.SZROUTE_BASE_URL ?? "http://127.0.0.1:21128";
    ...
    return fetch(url, {
      ...
      headers: {
        ...
        "x-szroute-source": "agent-bridge",
        "x-szroute-agent": this.agentId,
