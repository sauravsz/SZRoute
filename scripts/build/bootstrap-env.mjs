 ...
 *   3. Preferred config .env  (DATA_DIR/.env -> ~/.szroute/.env -> ./.env)
...
function writeEnvFile(filePath, env) {
  const lines = [
    "# Auto-generated by SZRoute bootstrap — do not delete",
...
export function bootstrapEnv({ dataDirOverride, quiet = false } = {}) {
  ...
  // This keeps run-next / run-standalone consistent with `bin/szroute.mjs`.
  //
  ...
  if (needsPersist) {
    merged.SZROUTE_BOOTSTRAPPED = "true";
  }
        ...
        if (row) {
          ...
          if (ciphertext?.startsWith("enc:v1:")) {
            ...
            if (parts.length >= 5) {
              ...
              // Legacy salt (fallback): scryptSync(secret, "szroute-field-encryption-v1", 32)
              const legacySalt = "szroute-field-encryption-v1";
              ...
              if (!keyMatched) {
                ...
                log(
                  "   Either restore your previous key via ~/.szroute/server.env or ~/.szroute/.env,"
                );
                log(
                  "   or run: szroute reset-encrypted-columns --force  (wipes credentials, keeps provider config)"
                );
              }
            }
          }
