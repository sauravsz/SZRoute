...
const DEFAULT_URL = "http://127.0.0.1:21128/login";
export const LINUX_EXECUTABLE_NAMES = ["szroute-desktop", "szroute", "SZRoute"];
...
function discoverMacExecutable() {
  ...
  const candidates = [
    ...
    join(distDir, "mac", "SZRoute.app", "Contents", "MacOS", "SZRoute"),
    join(distDir, "mac-arm64", "SZRoute.app", "Contents", "MacOS", "SZRoute"),
  ];
...
function discoverWindowsExecutable() {
  const distDir = join(ROOT, "electron", "dist-electron");
  const candidates = [
    join(distDir, "win-unpacked", "SZRoute.exe"),
    join(distDir, "win-x64-unpacked", "SZRoute.exe"),
    join(distDir, "win-arm64-unpacked", "SZRoute.exe"),
  ];

  return (
    candidates.find((candidate) => existsSync(candidate)) ||
    findExecutableByName(distDir, ["SZRoute.exe"]) ||
    candidates[0]
  );
}
...
async function assertPortIsFree(url) {
  try {
    ...
    throw new Error(
      `Smoke URL already responded with HTTP ${response.status}: ${url}. Stop the existing SZRoute process before running this check.`
    );
  } catch (error) {
...
async function ensureSmokeEnvDirs(smokeEnv, dataDir) {
  ...
  if (platform() === "win32" && smokeEnv.APPDATA) {
    for (const subdir of ["szroute-desktop", "SZRoute", "szroute"]) {
...
async function main() {
  ...
  if (!existsSync(appExecutable)) {
    throw new Error(
      `Packaged SZRoute executable not found at ${appExecutable}. Build it first with \`npm run build:<target> --prefix electron\` or set ELECTRON_SMOKE_APP_EXECUTABLE.`
    );
  }
  ...
  const dataDir =
    ...
    (await mkdtemp(join(tmpdir(), "szroute-electron-smoke-")));
  const removeDataDir =
