...
const CRYPTO_SECRETS = {
  ...
  // Generated in bin/szroute.mjs:ensureStorageEncryptionKey() and persisted to
  // ~/.szroute/.env to survive across upgrades. This prevents credential loss
  // when upgrading SZRoute (issue #1622).
  MACHINE_ID_SALT: () => `szroute-${randomBytes(8).toString("hex")}`,
};
 ...
 * It's generated at server startup in bin/szroute.mjs and persisted to
 * ~/.szroute/.env to survive across upgrades.
 * @see https://github.com/sauravsz/SZRoute/issues/1622
 */
...
function resolveDataDir(env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return join(appData, "szroute");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return join(resolve(xdg), "szroute");
        ...
        if (ENCRYPTION_BOUND_KEYS.has(key) && dbHasEncrypted) {
          log(
            `⚠️  ${key} NOT generated — encrypted credentials exist in DB. ` +
              `Restore your previous key via ~/.szroute/server.env, ~/.szroute/.env, ` +
