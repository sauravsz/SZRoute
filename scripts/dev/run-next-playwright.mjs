...
const bootstrapMode = process.env.SZROUTE_E2E_BOOTSTRAP_MODE || "auth";
const playwrightPassword =
  process.env.SZROUTE_E2E_PASSWORD || process.env.INITIAL_PASSWORD || "szroute-e2e-password";
const testServerEnv = {
  ...
  NEXT_PUBLIC_SZROUTE_E2E_MODE: process.env.NEXT_PUBLIC_SZROUTE_E2E_MODE || "1",
  SZROUTE_DISABLE_BACKGROUND_SERVICES:
    process.env.SZROUTE_DISABLE_BACKGROUND_SERVICES || "true",
  SZROUTE_DISABLE_TOKEN_HEALTHCHECK: process.env.SZROUTE_DISABLE_TOKEN_HEALTHCHECK || "true",
  SZROUTE_DISABLE_LOCAL_HEALTHCHECK: process.env.SZROUTE_DISABLE_LOCAL_HEALTHCHECK || "true",
  SZROUTE_HIDE_HEALTHCHECK_LOGS: process.env.SZROUTE_HIDE_HEALTHCHECK_LOGS || "true",
  ...(bootstrapMode === "open"
    ? {
        INITIAL_PASSWORD: "",
        SZROUTE_E2E_PASSWORD: "",
        SZROUTE_API_KEY: "",
      }
    : {
        INITIAL_PASSWORD: playwrightPassword,
        SZROUTE_E2E_PASSWORD: playwrightPassword,
      }),
  ...(process.env.SZROUTE_USE_TURBOPACK
    ? {
        SZROUTE_USE_TURBOPACK: process.env.SZROUTE_USE_TURBOPACK,
      }
    : {}),
};

export function shouldUseWebpackForPlaywrightDev({ mode, env }) {
  return mode === "dev" && env.SZROUTE_USE_TURBOPACK !== "1";
}
...
async function runBuildForStart() {
  ...
  if (process.env.SZROUTE_PLAYWRIGHT_SKIP_BUILD === "1") return;
