#!/usr/bin/env node
/**
 * Post-install warm-up for SZRoute native runtimes.
 *
 * Runs after the main `scripts/build/postinstall.mjs` binary-copy step.
 * Tries to pre-resolve better-sqlite3 into ~/.szroute/runtime/ so that
...
if (
  process.env.SZROUTE_SKIP_POSTINSTALL === "1" ||
  process.env.CI === "true" ||
  process.env.CI === "1"
) {
  process.exit(0);
...
(async () => {
  try {
    ...
    console.warn(`[szroute] postinstall warm-up skipped: ${err?.message ?? err}`);
  }
})();
