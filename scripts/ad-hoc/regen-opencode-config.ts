 ...
 * SZRoute /v1/models catalog. Run after a catalog change to refresh
 * the opencode client.
 *
...
const baseURL = process.env.SZROUTE_URL ?? "http://localhost:21128";
const apiKey = process.env.SZROUTE_KEY ?? process.env.OPENCODE_API_KEY ?? "";

if (!apiKey) {
  console.error(
    "SZROUTE_KEY (or OPENCODE_API_KEY) env var is required. " +
      "Find it in SZRoute dashboard → Settings → API Keys."
  );
  process.exit(1);
}
