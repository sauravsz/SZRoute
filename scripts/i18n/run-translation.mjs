#!/usr/bin/env node
/**
 * SZRoute — Docs translation pipeline (hash-based, incremental).
 *
 ...
 *   SZROUTE_TRANSLATION_API_URL     e.g. https://cloud.szroute.dev/v1
 *   SZROUTE_TRANSLATION_API_KEY     bearer token (kept out of logs)
 *   SZROUTE_TRANSLATION_MODEL       e.g. cx/gpt-5.4-mini
 *   SZROUTE_TRANSLATION_TIMEOUT_MS  optional, default 60000
 *   SZROUTE_TRANSLATION_CONCURRENCY optional, default 4
 */
...
function backendConfig() {
  const apiUrl = requireEnv("SZROUTE_TRANSLATION_API_URL").replace(/\/$/, "");
  const apiKey = requireEnv("SZROUTE_TRANSLATION_API_KEY");
  const model = requireEnv("SZROUTE_TRANSLATION_MODEL");
  const timeoutMs = Number(process.env.SZROUTE_TRANSLATION_TIMEOUT_MS || 60000);
...
async function main() {
  ...
  if (!opts.dryRun) {
    ...
    else backend.concurrency = Number(process.env.SZROUTE_TRANSLATION_CONCURRENCY || 4);
    logInfo(
      `backend: ${backend.apiUrl} (model=${backend.model}, concurrency=${backend.concurrency}, timeout=${backend.timeoutMs}ms)`
    );
  } else {
    const apiUrl = (process.env.SZROUTE_TRANSLATION_API_URL || "").replace(/\/$/, "");
