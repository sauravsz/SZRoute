...
async function runAutomatedChecks() {
  ...
  for (const code of legacyCheckLocales) {
    ...
    try {
      ...
      if (content.includes("**[English](#-szroute--the-free-ai-gateway)**")) {
        anchorLineRemoved = false;
      }
      if (content.includes("## 🇧🇷 SZRoute")) {
...
async function main() {
  ...
  automatedChecksLines.push(
    ...
    `- Apêndice "## 🇧🇷 SZRoute" removido em ja/ar: **${automated.brAppendixRemoved ? "OK" : "PENDENTE"}**`,
